// C / strencodings.h14-2018 The Bitcoin Core developers
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
        ...
        if (abs64(nMedian) <= std::max<int64_t>(0, gArgs.GetArg("-maxtimeadjustment", DEFAULT_MAX_TIME_ADJUSTMENT))) {
            ...
            if (!fDone) {
                ...
                if (!fMatch) {
                    fDone = true;
                    std::string strMessage = tfm::format(_("Please check that your computer's date and time are correct! If your clock is wrong, %s will not work properly."), _(PACKAGE_NAME));
