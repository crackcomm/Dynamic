// C / time.ht (c) 2009-2010 Satoshi Nakamoto
...
size_t CTxMemPoolEntry::GetTxSize() const { return GetVirtualTransactionSize(nTxWeight, sigOpCost); }
    ...
    bool fSearchForParents /* = true */) const
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (piter) {
                ...
                if (parentHashes.size() + 1 > limitAncestorCount) {
                    errString = tfm::format("too many unconfirmed parents [limit: %u]", limitAncestorCount);
                    return false;
                }
            }
        }
    ...
    while (!parentHashes.empty()) {
        ...
        if (stageit->GetSizeWithDescendants() + entry.GetTxSize() > limitDescendantSize) {
            errString = tfm::format(
                "exceeds descendant size limit for tx %s [limit: %u]", stageit->GetTx().GetHash().ToString(), limitDescendantSize);
            return false;
        } else if (stageit->GetCountWithDescendants() + 1 > limitDescendantCount) {
            errString =
                tfm::format("too many descendants for tx %s [limit: %u]", stageit->GetTx().GetHash().ToString(), limitDescendantCount);
            return false;
        } else if (totalSizeWithAncestors > limitAncestorSize) {
            errString = tfm::format("exceeds ancestor size limit [limit: %u]", limitAncestorSize);
            return false;
        }
        ...
        for (txiter phash : setMemPoolParents) {
            ...
            if (parentHashes.size() + setAncestors.size() + 1 > limitAncestorCount) {
                errString = tfm::format("too many unconfirmed ancestors [limit: %u]", limitAncestorCount);
                return false;
            }
        }
    }
...
{
    LOCK(cs);
    // Estimate the overhead of mapTx to be 12 pointers + an allocation, as no exact formula for boost::multi_index_contained is
    // implemented.
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 12 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapNextTx) +
           memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(mapLinks) + memusage::DynamicUsage(vTxHashes) + cachedInnerUsage;
}
