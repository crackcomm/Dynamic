...
private:
    ...
    bool SelectCoins(const std::vector<COutput>& vAvailableCoins,
        ...
        bool fUseInstantSend = true) const;
...
public:
    ...
    // Coin selection
    bool SelectCoinsByDenominations(int nDenom,
        CAmount nValueMin,
        CAmount nValueMax,
    ...
    bool SelectCoinsMix(CAmount nValueMin,
        const CAmount nValueMax,
        std::vector<CTxIn>& vecTxInRet,
        CAmount& nValueRet,
        int nPrivateSendRoundsMin,
        int nPrivateSendRoundsMax) const;
    bool SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet,
        bool fSkipDenominated = true,
        bool fAnonymizable = true,
        bool fSkipUnconfirmed = true) const;

    /// Get 1000DYN output and keys which can be used for the Dynode
    bool GetDynodeOutpointAndKeys(COutPoint& outpointRet,
        CPubKey& pubKeyRet,
        CKey& keyRet,
    ...
    bool CommitTransaction(CWalletTx& wtxNew,
        ...
        const std::string& strCommand = "tx");
