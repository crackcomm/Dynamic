...
struct CompareLastPaidBlock {
    bool operator()(const std::pair<int, const CDynode*>& t1, const std::pair<int, const CDynode*>& t2) const
    {
...
struct CompareScoreDN {
    bool operator()(const std::pair<arith_uint256, const CDynode*>& t1, const std::pair<arith_uint256, const CDynode*>& t2) const
    {
        return (t1.first != t2.first) ? (t1.first < t2.first) : (t1.second->outpoint < t2.second->outpoint);
    }
};
...
{
    bool operator()(const CDynode* t1, const CDynode* t2) const { return t1->addr < t2->addr; }
};
...
void CDynodeMan::Check()
{
    ...
    LogPrint(BCLog::DYNODE, "CDynodeMan::Check -- nLastSentinelPingTime=%d, IsSentinelPingActive()=%d\n", nLastSentinelPingTime,
        ...
        while (it != mapDynodes.end()) {
            ...
            if (it->second.IsOutpointSpent()) {
                LogPrint(BCLog::DYNODE, "CDynodeMan::CheckAndRemove -- Removing Dynode: %s  addr=%s  %i now\n", it->second.GetStateString(),
                    it->second.addr.ToString(), size() - 1);
                ...
                bool fAsk = (nAskForDnbRecovery > 0) && dynodeSync.IsSynced() && it->second.IsNewStartRequired() &&
                            !IsDnbRecoveryRequested(hash) && !IsArgSet("-connect");
                if (fAsk) {
                    ...
                    for (int i = 0; setRequested.size() < DNB_RECOVERY_QUORUM_TOTAL && i < (int)vecDynodeRanks.size(); i++) {
                        // avoid banning
                        if (mWeAskedForDynodeListEntry.count(it->first) &&
                            mWeAskedForDynodeListEntry[it->first].count(vecDynodeRanks[i].second.addr))
                            continue;
...
void CDynodeMan::PsegUpdate(CNode* pnode, CConnman& connman)
{
    ...
    LogPrint(BCLog::DYNODE, "CDynodeMan::PsegUpdate -- asked %s for the list\n", pnode->addr.ToString());
}
...
dynode_info_t CDynodeMan::FindRandomNotInVec(const std::vector<COutPoint>& vecToExclude, int nProtocolVersion)
{
    ...
    // shuffle pointers
    std::random_shuffle(vpDynodesShuffled.begin(), vpDynodesShuffled.end(), GetRandInt);
    bool fExclude;
...
void CDynodeMan::ProcessPendingDnbRequests(CConnman& connman)
{
    ...
    while (itPendingDNB != mapPendingDNB.end()) {
        bool fDone = connman.ForNode(itPendingDNB->first, [&](CNode* pnode) {
            ...
            for (auto& nHash : itPendingDNB->second.second) {
                if (nHash != uint256()) {
                    ...
                    LogPrint(BCLog::DYNODE, "-- asking for dnb %s from addr=%s\n", nHash.ToString(), pnode->addr.ToString());
                }
            }
        ...
        if (fDone || (GetTime() - nTimeAdded > 15)) {
            if (!fDone) {
                LogPrint(BCLog::DYNODE, "CDynodeMan::%s -- failed to connect to %s\n", __func__, itPendingDNB->first.ToString());
            }
...
void CDynodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::DNANNOUNCE) { // Dynode Broadcast
        ...
        LogPrint(BCLog::DYNODE, "PSEG -- Dynode list, Dynode=%s\n", dynodeOutpoint.ToStringShort());
...
void CDynodeMan::SyncSingle(CNode* pnode, const COutPoint& outpoint, CConnman& connman)
{
    ...
    if (it != mapDynodes.end()) {
        ...
        LogPrint(BCLog::DYNODE, "CDynodeMan::%s -- Sending Dynode entry: Dynode=%s  addr=%s\n", __func__, outpoint.ToStringShort(),
...
void CDynodeMan::SyncAll(CNode* pnode, CConnman& connman)
{
    ...
    for (const auto& dnpair : mapDynodes) {
        if (Params().RequireRoutableExternalIP() && (dnpair.second.addr.IsRFC1918() || dnpair.second.addr.IsLocal()))
            continue; // do not send local network Dynode
        // NOTE: send Dynode regardless of its current state, the other node will need it to verify old votes.
        LogPrint(BCLog::DYNODE, "CDynodeMan::%s -- Sending Dynode entry: Dynode=%s  addr=%s\n", __func__, dnpair.first.ToStringShort(),
            dnpair.second.addr.ToString());
    ...
    for (auto& rankPair : vecDynodeRanks) {
        ...
        if (rankPair.second.outpoint == activeDynode.outpoint) {
            ...
            LogPrint(BCLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Found self at rank %d/%d, verifying up to %d Dynodes\n", nMyRank,
                nRanksTotal, (int)MAX_POSE_CONNECTIONS);
            break;
        }
    }
    ...
    while (it != vecDynodeRanks.end()) {
        if (it->second.IsPoSeVerified() || it->second.IsPoSeBanned()) {
            LogPrint(BCLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Already %s%s%s Dynode %s address %s, skipping...\n",
                it->second.IsPoSeVerified() ? "verified" : "", it->second.IsPoSeVerified() && it->second.IsPoSeBanned() ? " and " : "",
                it->second.IsPoSeBanned() ? "banned" : "", it->second.outpoint.ToStringShort(), it->second.addr.ToString());
...
{
    if (netfulfilledman.HasFulfilledRequest(addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-request")) {
...
void CDynodeMan::ProcessPendingDnvRequests(CConnman& connman)
{
    ...
    while (itPendingDNV != mapPendingDNV.end()) {
        bool fDone = connman.ForNode(itPendingDNV->first, [&](CNode* pnode) {
            netfulfilledman.AddFulfilledRequest(pnode->addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-request");
            // use random nonce, store it and require node to reply with correct one later
            mWeAskedForVerification[pnode->addr] = itPendingDNV->second.second;
            LogPrint(
                BCLog::DYNODE, "-- verifying node using nonce %d addr=%s\n", itPendingDNV->second.second.nonce, pnode->addr.ToString());
            CNetMsgMaker msgMaker(
                pnode->GetSendVersion()); // TODO this gives a warning about version not being set (we should wait for VERSION exchange)
...
void CDynodeMan::SendVerifyReply(CNode* pnode, CDynodeVerification& dnv, CConnman& connman)
{
    ...
    if (netfulfilledman.HasFulfilledRequest(pnode->addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-reply")) {
    ...
    if (!GetBlockHash(blockHash, dnv.nBlockHeight)) {
        LogPrintf(
            "CDynodeMan::SendVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", dnv.nBlockHeight, pnode->id);
        return;
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        std::string strMessage = tfm::format("%s%d%s", activeDynode.service.ToString(false), dnv.nonce, blockHash.ToString());
    ...
    netfulfilledman.AddFulfilledRequest(pnode->addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-reply");
}
...
void CDynodeMan::ProcessVerifyReply(CNode* pnode, CDynodeVerification& dnv)
{
    ...
    if (!netfulfilledman.HasFulfilledRequest(pnode->addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-request")) {
        LogPrintf(
            "CDynodeMan::ProcessVerifyReply -- ERROR: we didn't ask for verification of %s, peer=%d\n", pnode->addr.ToString(), pnode->id);
        Misbehaving(pnode->id, 20);
        return;
    }
    ...
    if (!GetBlockHash(blockHash, dnv.nBlockHeight)) {
        ...
        LogPrintf(
            "DynodeMan::ProcessVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", dnv.nBlockHeight, pnode->id);
        return;
    }

    // we already verified this address, why node is spamming?
    if (netfulfilledman.HasFulfilledRequest(pnode->addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-done")) {
...

    {
        ...
        std::string strMessage1 = tfm::format("%s%d%s", pnode->addr.ToString(false), dnv.nonce, blockHash.ToString());
        ...
        for (auto& dnpair : mapDynodes) {
            if (CAddress(dnpair.second.addr, NODE_NETWORK) == pnode->addr) {
                ...
                if (fFound) {
                    ...
                    netfulfilledman.AddFulfilledRequest(pnode->addr, tfm::format("%s", NetMsgType::DNVERIFY) + "-done");
                    ...
                    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
                        ...
                        std::string strMessage2 = tfm::format("%s%d%s%s%s", dnv.addr.ToString(false), dnv.nonce, blockHash.ToString(),
...
void CDynodeMan::ProcessVerifyBroadcast(CNode* pnode, const CDynodeVerification& dnv)
{
    ...
    if (dnv.nBlockHeight < nCachedBlockHeight - MAX_POSE_BLOCKS) {
        LogPrint(BCLog::DYNODE, "CDynodeMan::ProcessVerifyBroadcast -- Outdated: current block %d, verification block %d, peer=%d\n",
            nCachedBlockHeight, dnv.nBlockHeight, pnode->id);
        return;
    }

    if (dnv.dynodeOutpoint1 == dnv.dynodeOutpoint2) {
        LogPrint(BCLog::DYNODE, "CDynodeMan::ProcessVerifyBroadcast -- ERROR: same outpoints %s, peer=%d\n",
    ...
    if (!GetDynodeRank(dnv.dynodeOutpoint2, nRank, dnv.nBlockHeight, MIN_POSE_PROTO_VERSION)) {
        LogPrint(BCLog::DYNODE, "CDynodeMan::ProcessVerifyBroadcast -- Can't calculate rank for Dynode %s\n",
            dnv.dynodeOutpoint2.ToStringShort());
        return;
    }

    if (nRank > MAX_POSE_RANK) {
        LogPrint(BCLog::DYNODE, "CDynodeMan::ProcessVerifyBroadcast -- Dynode %s is not in top %d, current rank %d, peer=%d\n",
...

    {
        ...
        if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
            ...
            std::string strMessage1 = tfm::format("%s%d%s", dnv.addr.ToString(false), dnv.nonce, blockHash.ToString());
            std::string strMessage2 = tfm::format("%s%d%s%s%s", dnv.addr.ToString(false), dnv.nonce, blockHash.ToString(),
                dnv.dynodeOutpoint1.ToStringShort(), dnv.dynodeOutpoint2.ToStringShort());
        ...
        if (nCount)
            LogPrintf(
                "CDynodeMan::ProcessVerifyBroadcast -- PoSe score increased for %d fake Dynodes, addr %s\n", nCount, pdn1->addr.ToString());
    }
}
...
std::string CDynodeMan::ToString() const
{
    ...
    info << "Dynodes: " << (int)mapDynodes.size() << ", peers who asked us for Dynode list: " << (int)mAskedUsForDynodeList.size()
         << ", peers we asked for Dynode list: " << (int)mWeAskedForDynodeList.size()
         << ", entries in Dynode list we asked for: " << (int)mWeAskedForDynodeListEntry.size() << ", nPsqCount: " << (int)nPsqCount;
...

    {
            ...
            if (pfrom && IsDnbRecoveryRequested(hash) && GetTime() < mDnbRecoveryRequests[hash].first) {
                ...
                if (mDnbRecoveryRequests[hash].second.count(pfrom->addr)) {
                    ...
                    if (dnb.lastPing.sigTime > mapSeenDynodeBroadcast[hash].second.lastPing.sigTime) {
                        ...
                        dnTemp.Check();
                        LogPrint(BCLog::DYNODE,
                            "CDynodeMan::CheckDnbAndUpdateDynodeList -- dnb=%s seen request, addr=%s, better lastPing: %d min ago, "
                            "projected dn state: %s\n",
                            hash.ToString(), pfrom->addr.ToString(), (GetAdjustedTime() - dnb.lastPing.sigTime) / 60,
                            dnTemp.GetStateString());
        ...
        if (pdn) {
            ...
            if (!dnb.Update(pdn, nDos, connman)) {
                LogPrint(
                    BCLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Update() failed, dynode=%s\n", dnb.outpoint.ToStringShort());
                return false;
            }
    ...
    if (dnb.CheckOutpoint(nDos)) {
        ...
        if (fDynodeMode && dnb.pubKeyDynode == activeDynode.pubKeyDynode) {
            ...
            if (dnb.nProtocolVersion == PROTOCOL_VERSION) {
                ...
                LogPrintf("CDynodeMan::CheckDnbAndUpdateDynodeList -- wrong PROTOCOL_VERSION, re-activate your DN: message "
                          "nProtocolVersion=%d  PROTOCOL_VERSION=%d\n",
...
void CDynodeMan::UpdateLastPaid(const CBlockIndex* pindex)
{
    ...
    LogPrint(BCLog::DYNODE, "CDynodeMan::UpdateLastPaid -- nCachedBlockHeight=%d, nLastRunBlockHeight=%d, nMaxBlocksToScanBack=%d\n",
...
void CDynodeMan::UpdatedBlockTip(const CBlockIndex* pindex)
{
    ...
    LogPrint(BCLog::DYNODE, "CDynodeMan::UpdatedBlockTip -- nCachedBlockHeight=%d\n", nCachedBlockHeight);
...
void CDynodeMan::WarnDynodeDaemonUpdates()
{
    ...
    if (nUpdatedDynodes != size()) {
        strWarning = tfm::format(_("Warning: At least %d of %d dynodes are running on a newer software version. Please check latest "
                                   "releases, you might need to update too."),
            nUpdatedDynodes, size());
    } else {
        // someone was postponing this update for way too long probably
        strWarning = tfm::format(_("Warning: Every dynode (out of %d known ones) is running on a newer software version. Please check "
                                   "latest releases, it's very likely that you missed a major/critical update."),
            size());
    }
