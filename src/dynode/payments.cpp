...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
    ...
    if (nBlockHeight < consensusParams.nSuperblockStartBlock) {
        ...
        // LogPrint(BCLog::GOBJECT, "IsBlockValueValid -- Block is not in budget cycle window, checking block value against block
        // reward\n");
        if (!isBlockRewardValueMet) {
            strErrorRet = tfm::format(
    ...
    LogPrint(
        BCLog::GOBJECT, "block.vtx[0].GetValueOut() %lld <= nSuperblockMaxValue %lld\n", block.vtx[0]->GetValueOut(), nSuperblockMaxValue);
    ...
    if (!dynodeSync.IsSynced()) {
        ...
        if (CSuperblock::IsValidBlockHeight(nBlockHeight)) {
            ...
            if (!isSuperblockMaxValueMet) {
                strErrorRet = tfm::format("coinbase pays too much at height %d (actual=%d vs limit=%d), exceeded superblock max value",
                    nBlockHeight, block.vtx[0]->GetValueOut(), nSuperblockMaxValue);
            }
            return isSuperblockMaxValueMet;
        }
        if (!isBlockRewardValueMet) {
            strErrorRet = tfm::format("coinbase pays too much at height %d (actual=%d vs limit=%d), exceeded block reward, only regular "
                                      "blocks are allowed at this height",
                nBlockHeight, block.vtx[0]->GetValueOut(), blockReward);
        }
    ...
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)) {
        if (CSuperblockManager::IsSuperblockTriggered(nBlockHeight)) {
            ...
            strErrorRet = tfm::format("invalid superblock detected at height %d", nBlockHeight);
            return false;
        }
        ...
        if (!isBlockRewardValueMet) {
            strErrorRet =
                tfm::format("coinbase pays too much at height %d (actual=%d vs limit=%d), exceeded block reward, superblocks are disabled",
                    nBlockHeight, block.vtx[0]->GetValueOut(), blockReward);
        }
    }
    ...
    std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    LogPrint(BCLog::DNPAYMENTS, "FillBlockPayments -- nBlockHeight %d blockReward %lld txoutDynodeRet %s txNew %s", nBlockHeight,
        blockReward, txoutDynodeRet.ToString(), txNew.ToString());
}
...
{
    return sporkManager.IsSporkActive(SPORK_10_DYNODE_PAY_UPDATED_NODES) ? MIN_DYNODE_PAYMENT_PROTO_VERSION_2 :
...
void CDynodePayments::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::DYNODEPAYMENTSYNC) { // Dynode Payments Request Sync
        ...
        if (pfrom->nVersion < GetMinDynodePaymentsProto()) {
            LogPrint(BCLog::DNPAYMENTS, "DYNODEPAYMENTSYNC -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion())
                                           .Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                                               tfm::format("Version must be %d or greater", GetMinDynodePaymentsProto())));
            return;
        }
        ...
        if (pfrom->nVersion < GetMinDynodePaymentsProto()) {
            LogPrint(BCLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion())
                                           .Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                                               tfm::format("Version must be %d or greater", GetMinDynodePaymentsProto())));
            return;
...

        {
            ...
            if (!res.second && res.first->second.IsVerified()) {
                LogPrint(BCLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- hash=%s, nBlockHeight=%d/%d seen\n", nHash.ToString(), vote.nBlockHeight,
                    nCachedBlockHeight);
                return;
            }
        ...
        LogPrint(BCLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- vote: address=%s, nBlockHeight=%d, nHeight=%d, prevout=%s, hash=%s new\n",
...
uint256 CDynodePaymentVote::GetSignatureHash() const { return SerializeHash(*this); }
...
bool CDynodePaymentVote::Sign()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        std::string strMessage = dynodeOutpoint.ToStringShort() + std::to_string(nBlockHeight) + ScriptToAsmStr(payee);
...
bool CDynodePayments::AddOrUpdatePaymentVote(const CDynodePaymentVote& vote)
{
    ...
    LogPrint(BCLog::DNPAYMENTS, "CDynodePayments::AddOrUpdatePaymentVote -- added, hash=%s\n", nVoteHash.ToString());
...
std::string CDynodeBlockPayees::GetRequiredPaymentsString() const
{
    ...
    for (const auto& payee : vecPayees) {
        ...
        strRequiredPayments += tfm::format("%s:%d", address2.ToString(), payee.GetVoteCount());
    }
...
void CDynodePayments::CheckAndRemove()
{
    ...
    while (it != mapDynodePaymentVotes.end()) {
        ...
        if (nCachedBlockHeight - vote.nBlockHeight > nLimit) {
            LogPrint(
                BCLog::DNPAYMENTS, "CDynodePayments::CheckAndRemove -- Removing old Dynode payment: nBlockHeight=%d\n", vote.nBlockHeight);
...
bool CDynodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman& connman) const
{
    ...
    if (!dnodeman.GetDynodeInfo(dynodeOutpoint, dnInfo)) {
        strError = tfm::format("Unknown dynode=%s", dynodeOutpoint.ToStringShort());
    ...
    if (dnInfo.nProtocolVersion < nMinRequiredProtocol) {
        strError = tfm::format(
            "Dynode protocol is too old: nProtocolVersion=%d, nMinRequiredProtocol=%d", dnInfo.nProtocolVersion, nMinRequiredProtocol);
        return false;
    }
    ...
    if (!dnodeman.GetDynodeRank(dynodeOutpoint, nRank, nBlockHeight - 101, nMinRequiredProtocol)) {
        LogPrint(BCLog::DNPAYMENTS, "CDynodePaymentVote::IsValid -- Can't calculate rank for dynode %s\n", dynodeOutpoint.ToStringShort());
        return false;
    }
    ...
    if (nRank > DNPAYMENTS_SIGNATURES_TOTAL) {
        ...
        strError = tfm::format("Dynode %s is not in the top %d (%d)", dynodeOutpoint.ToStringShort(), DNPAYMENTS_SIGNATURES_TOTAL, nRank);
        ...
        if (nRank > DNPAYMENTS_SIGNATURES_TOTAL * 2 && nBlockHeight > nValidationHeight) {
            LOCK(cs_main);
            strError =
                tfm::format("Dynode %s is not in the top %d (%d)", dynodeOutpoint.ToStringShort(), DNPAYMENTS_SIGNATURES_TOTAL * 2, nRank);
...
void CDynodePayments::CheckBlockVotes(int nBlockHeight)
{
    ...
    debugStr += tfm::format("CDynodePayments::CheckBlockVotes -- nBlockHeight=%d,\n  Expected voting DNs:\n", nBlockHeight);
            ...
            for (const auto& p : it->second.vecPayees) {
                for (const auto& voteHash : p.GetVoteHashes()) {
                    ...
                    if (itVote == mapDynodePaymentVotes.end()) {
                        debugStr += tfm::format("    - could not find vote %s\n", voteHash.ToString());
                        continue;
                    }
...
void CDynodePaymentVote::Relay(CConnman& connman) const
{
    ...
    if (!dynodeSync.IsSynced()) {
        LogPrint(BCLog::DNPAYMENTS, "CDynodePayments::Relay -- won't relay until fully synced\n");
        return;
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyDynode, vchSig, strError)) {
            ...
            std::string strMessage =
                dynodeOutpoint.ToStringShort() + boost::lexical_cast<std::string>(nBlockHeight) + ScriptToAsmStr(payee);
        ...
        std::string strMessage = dynodeOutpoint.ToStringShort() + boost::lexical_cast<std::string>(nBlockHeight) + ScriptToAsmStr(payee);
...
int CDynodePayments::GetStorageLimit() const { return std::max(int(dnodeman.size() * nStorageCoeff), nMinBlocksToStore); }
...
void CDynodePayments::UpdatedBlockTip(const CBlockIndex* pindex, CConnman& connman)
{
    ...
    LogPrint(BCLog::DNPAYMENTS, "CDynodePayments::UpdatedBlockTip -- nCachedBlockHeight=%d\n", nCachedBlockHeight);
