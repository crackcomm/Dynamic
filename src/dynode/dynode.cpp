...
CDynode::CDynode(CService addr, COutPoint outpoint, CPubKey pubKeyCollateralAddress, CPubKey pubKeyDynode, int nProtocolVersionIn)
    : dynode_info_t{DYNODE_ENABLED, nProtocolVersionIn, GetAdjustedTime(), outpoint, addr, pubKeyCollateralAddress, pubKeyDynode}
    , fAllowMixingTx(true)
{
}
...
CDynode::CDynode(const CDynodeBroadcast& dnb)
    : dynode_info_t{dnb.nActiveState, dnb.nProtocolVersion, dnb.sigTime, dnb.outpoint, dnb.addr, dnb.pubKeyCollateralAddress,
          dnb.pubKeyDynode}
...
bool CDynode::UpdateFromNewBroadcast(CDynodeBroadcast& dnb, CConnman& connman)
{
    ...
    if (fDynodeMode && pubKeyDynode == activeDynode.pubKeyDynode) {
        ...
        if (nProtocolVersion == PROTOCOL_VERSION) {
            ...
            LogPrintf("CDynode::UpdateFromNewBroadcast -- wrong PROTOCOL_VERSION, re-activate your DN: message nProtocolVersion=%d  "
                      "PROTOCOL_VERSION=%d\n",
...
void CDynode::Check(bool fForce)
{
    ...
    if (!fWaitForPing || fOurDynode) {
        ...
        LogPrint(BCLog::DYNODE, "CDynode::Check -- outpoint=%s, GetAdjustedTime()=%d, fSentinelPingExpired=%d\n", outpoint.ToStringShort(),
            GetAdjustedTime(), fSentinelPingExpired);
...
bool CDynode::IsValidNetAddr() { return IsValidNetAddr(addr); }
...
std::string CDynode::GetStateString() const { return StateToString(nActiveState); }
...
#ifdef ENABLE_WALLET
bool CDynodeBroadcast::Create(const std::string strService,
    const std::string strKeyDynode,
    const std::string strTxHash,
    const std::string strOutputIndex,
    std::string& strErrorRet,
    CDynodeBroadcast& dnbRet,
    bool fOffline)
{
    COutPoint outpoint;
    ...
    if (!CMessageSigner::GetKeysFromSecret(strKeyDynode, keyDynodeNew, pubKeyDynodeNew))
        return Log(tfm::format("Invalid Dynode key %s", strKeyDynode));
    ...
    if (!pwalletMain->GetDynodeOutpointAndKeys(outpoint, pubKeyCollateralAddressNew, keyCollateralAddressNew, strTxHash, strOutputIndex))
        return Log(tfm::format("Could not allocate outpoint %s:%s for dynode %s", strTxHash, strOutputIndex, strService));

    CService service;
    if (!Lookup(strService.c_str(), service, 0, false))
        return Log(tfm::format("Invalid address %s for dynode.", strService));
    int mainnetDefaultPort = Params(CBaseChainParams::MAIN).GetDefaultPort();
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (service.GetPort() != mainnetDefaultPort)
            return Log(tfm::format(
                "Invalid port %u for dynode %s, only %d is supported on mainnet.", service.GetPort(), strService, mainnetDefaultPort));
    } else if (service.GetPort() == mainnetDefaultPort)
        return Log(tfm::format(
            "Invalid port %u for dynode %s, %d is the only supported on mainnet.", service.GetPort(), strService, mainnetDefaultPort));

    return Create(
        outpoint, service, keyCollateralAddressNew, pubKeyCollateralAddressNew, keyDynodeNew, pubKeyDynodeNew, strErrorRet, dnbRet);
}

bool CDynodeBroadcast::Create(const COutPoint& outpoint,
    const CService& service,
    const CKey& keyCollateralAddressNew,
    const CPubKey& pubKeyCollateralAddressNew,
    const CKey& keyDynodeNew,
    const CPubKey& pubKeyDynodeNew,
    std::string& strErrorRet,
    CDynodeBroadcast& dnbRet)
{
    ...
    LogPrint(BCLog::DYNODE, "CDynodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyDynodeNew.GetID() = %s\n",
        CDynamicAddress(pubKeyCollateralAddressNew.GetID()).ToString(), pubKeyDynodeNew.GetID().ToString());
    ...
    if (!dnp.Sign(keyDynodeNew, pubKeyDynodeNew))
        return Log(tfm::format("Failed to sign ping, dynode=%s", outpoint.ToStringShort()));
    ...
    if (!dnbRet.IsValidNetAddr())
        return Log(tfm::format("Invalid IP address, dynode=%s", outpoint.ToStringShort()));

    dnbRet.lastPing = dnp;
    if (!dnbRet.Sign(keyCollateralAddressNew))
        return Log(tfm::format("Failed to sign broadcast, dynode=%s", outpoint.ToStringShort()));
...
bool CDynodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    if (!IsValidNetAddr()) {
        LogPrintf(
            "CDynodeBroadcast::SimpleCheck -- Invalid addr, rejected: Dynode=%s  addr=%s\n", outpoint.ToStringShort(), addr.ToString());
        return false;
    }
...
bool CDynodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint(
            BCLog::DYNODE, "CDynodeBroadcast::CheckOutpoint -- Dynode UTXO should have 1000 DYN, dynode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }

    if (err == COLLATERAL_INVALID_PUBKEY) {
        LogPrint(BCLog::DYNODE, "CDynodeBroadcast::CheckOutpoint -- Dynode UTXO should match pubKeyCollateralAddress, dynode=%s\n",
            outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
    ...
    CBlockIndex* pRequredConfIndex =
        chainActive[nHeight + Params().GetConsensus().nDynodeMinimumConfirmations - 1]; // block where tx got nDynodeMinimumConfirmations
    if (pRequredConfIndex->GetBlockTime() > sigTime) {
        LogPrintf("CDynodeBroadcast::CheckOutpoint -- Bad sigTime %d (%d conf block is at %d) for Dynode %s %s\n", sigTime,
            Params().GetConsensus().nDynodeMinimumConfirmations, pRequredConfIndex->GetBlockTime(), outpoint.ToStringShort(),
            addr.ToString());
        return false;
    }
...
bool CDynodeBroadcast::Sign(const CKey& keyCollateralAddress)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        std::string strMessage = addr.ToString(false) + std::to_string(sigTime) + pubKeyCollateralAddress.GetID().ToString() +
                                 pubKeyDynode.GetID().ToString() + std::to_string(nProtocolVersion);
...
void CDynodeBroadcast::Relay(CConnman& connman) const
{
    ...
    if (!dynodeSync.IsSynced()) {
        LogPrint(BCLog::DYNODE, "CDynodeBroadcast::Relay -- won't relay until fully synced\n");
        return;
    }
...
uint256 CDynodePing::GetSignatureHash() const { return GetHash(); }
...
bool CDynodePing::CheckAndUpdate(CDynode* pdn, bool fFromNewBroadcast, int& nDos, CConnman& connman)
{
    ...
    if (!fFromNewBroadcast) {
        if (pdn->IsUpdateRequired()) {
            LogPrint(
                BCLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode protocol is outdated, Dynode=%s\n", dynodeOutpoint.ToStringShort());
            return false;
        }
    ...
    if (pdn->IsPingedWithin(DYNODE_MIN_DNP_SECONDS - 60, sigTime)) {
        LogPrint(
            BCLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode ping arrived too early, Dynode=%s\n", dynodeOutpoint.ToStringShort());
    ...
    LogPrint(
        BCLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode ping acceepted and relayed, Dynode=%s\n", dynodeOutpoint.ToStringShort());
    Relay(connman);
...
void CDynodePing::Relay(CConnman& connman)
{
    // Do not relay until fully synced
    if (!dynodeSync.IsSynced()) {
        LogPrint(BCLog::DYNODE, "CDynodePing::Relay -- won't relay until fully synced\n");
        return;
    }
