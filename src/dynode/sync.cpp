...
void CDynodeSync::BumpAssetLastTime(const std::string strFuncName)
{
    if (IsSynced() || IsFailed())
        return;
    nTimeLastBumped = GetTime();
    LogPrint(BCLog::DNSYNC, "CDynodeSync::BumpAssetLastTime -- %s\n", strFuncName);
}
...
void CDynodeSync::SwitchToNextAsset(CConnman& connman)
{
    ...
    case (DYNODE_SYNC_GOVERNANCE):
        ...
        connman.ForEachNode(CConnman::AllNodes, [](CNode* pnode) { netfulfilledman.AddFulfilledRequest(pnode->addr, "full-sync"); });
...
void CDynodeSync::ProcessTick(CConnman& connman)
{
    ...
    LogPrintf("CDynodeSync::ProcessTick -- nTick %d nRequestedDynodeAssets %d nRequestedDynodeAttempt %d nSyncProgress %f\n", nTick,
        nRequestedDynodeAssets, nRequestedDynodeAttempt, nSyncProgress);
        ...
        if (Params().NetworkIDString() == CBaseChainParams::REGTEST) {
            if (nRequestedDynodeAttempt <= 2) {
                ...
                if (pnode->nVersion == 70900) {
                    connman.PushMessage(
                        pnode, msgMaker.Make(NetMsgType::DYNODEPAYMENTSYNC, dnpayments.GetStorageLimit())); // sync payment votes
                } else {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            ...
            if (!netfulfilledman.HasFulfilledRequest(pnode->addr, "spork-sync")) {
                ...
                LogPrintf("CDynodeSync::ProcessTick -- nTick %d nRequestedDynodeAssets %d -- requesting sporks from peer %d\n", nTick,
                    nRequestedDynodeAssets, pnode->id);
            }
            ...
            if (nRequestedDynodeAssets == DYNODE_SYNC_LIST) {
                LogPrint(BCLog::DYNODE,
                    "CDynodeSync::ProcessTick -- nTick %d nRequestedDynodeAssets %d nTimeLastBumped %lld GetTime() %lld diff %lld\n", nTick,
                    nRequestedDynodeAssets, nTimeLastBumped, GetTime(), GetTime() - nTimeLastBumped);
                ...
                if (netfulfilledman.HasFulfilledRequest(pnode->addr, "governance-sync")) {
                    ...
                    if (nObjsLeftToAsk == 0) {
                        ...
                        if (GetTime() - nTimeNoObjectsLeft > DYNODE_SYNC_TIMEOUT_SECONDS &&
                            ...
                            LogPrintf(
                                "CDynodeSync::ProcessTick -- nTick %d nRequestedDynodeAssets %d -- asked for all objects, nothing to do\n",
                                nTick, nRequestedDynodeAssets);
...
{
    LogPrint(BCLog::DNSYNC, "CDynodeSync::AcceptedBlockHeader -- pindexNew->nHeight: %d\n", pindexNew->nHeight);
...
{
    LogPrint(BCLog::DNSYNC, "CDynodeSync::NotifyHeaderTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight,
        fInitialDownload);
...
{
    LogPrint(BCLog::DNSYNC, "CDynodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight,
        fInitialDownload);
    ...
    LogPrint(BCLog::DNSYNC,
        "CDynodeSync::NotifyHeaderTip -- pindexNew->nHeight: %d pindexBestHeader->nHeight: %d fInitialDownload=%d fReachedBestHeader=%d\n",
