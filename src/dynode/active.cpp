...
std::string CActiveDynode::GetStatus() const
{
    ...
    case ACTIVE_DYNODE_INPUT_TOO_NEW:
        return tfm::format("Dynode input must have at least %d confirmations", Params().GetConsensus().nDynodeMinimumConfirmations);
    case ACTIVE_DYNODE_NOT_CAPABLE:
...
bool CActiveDynode::SendDynodePing(CConnman& connman)
{
    ...
    dnp.fSentinelIsCurrent = (llabs(GetAdjustedTime() - nSentinelPingTime) < DYNODE_SENTINEL_PING_MAX_SECONDS);
...
void CActiveDynode::ManageStateInitial(CConnman& connman)
{
    ...
    if (!fListen) {
        ...
        nState = ACTIVE_DYNODE_NOT_CAPABLE;
        strNotCapableReason = "Dynode must accept connections from outside. Make sure listen configuration option is not overwritten by "
                              "some another parameter.";
    ...
    if (!fFoundLocal) {
        nState = ACTIVE_DYNODE_NOT_CAPABLE;
        strNotCapableReason = "Can't detect valid external address. Please consider using the externalip configuration option if problem "
                              "persists. Make sure to use IPv4 address only.";
        LogPrintf("CActiveDynode::ManageStateInitial -- %s: %s\n", GetStateString(), strNotCapableReason);
        return;
    }
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (service.GetPort() != mainnetDefaultPort) {
            nState = ACTIVE_DYNODE_NOT_CAPABLE;
            strNotCapableReason = tfm::format("Invalid port: %u - only %d is supported on mainnet.", service.GetPort(), mainnetDefaultPort);
            LogPrintf("CActiveDynode::ManageStateInitial -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
    } else if (service.GetPort() == mainnetDefaultPort) {
        nState = ACTIVE_DYNODE_NOT_CAPABLE;
        strNotCapableReason = tfm::format("Invalid port: %u - %d is only supported on mainnet.", service.GetPort(), mainnetDefaultPort);
        LogPrintf("CActiveDynode::ManageStateInitial -- %s: %s\n", GetStateString(), strNotCapableReason);
        return;
    }
...
void CActiveDynode::ManageStateRemote()
{
    LogPrint(BCLog::DYNODE,
        "CActiveDynode::ManageStateRemote -- Start status = %s, type = %s, pinger enabled = %d, pubKeyDynode.GetID() = %s\n", GetStatus(),
        fPingerEnabled, GetTypeString(), pubKeyDynode.GetID().ToString());
    ...
    if (dnodeman.GetDynodeInfo(pubKeyDynode, infoDn)) {
        ...
        if (service != infoDn.addr) {
            nState = ACTIVE_DYNODE_NOT_CAPABLE;
            strNotCapableReason = "Broadcasted IP doesn't match our external address. Make sure you issued a new broadcast if IP of this "
                                  "Dynode changed recently.";
            LogPrintf("CActiveDynode::ManageStateRemote -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
        if (!CDynode::IsValidStateForAutoStart(infoDn.nActiveState)) {
            nState = ACTIVE_DYNODE_NOT_CAPABLE;
            strNotCapableReason = tfm::format("Dynode in %s state", CDynode::StateToString(infoDn.nActiveState));
            LogPrintf("CActiveDynode::ManageStateRemote -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
