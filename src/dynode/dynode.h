...
inline bool operator!=(const CDynodePing& a, const CDynodePing& b) { return !(a == b); }
inline CDynodePing::operator bool() const { return *this != CDynodePing(); }
...
struct dynode_info_t {
    ...
    dynode_info_t(int activeState,
        int protoVer,
        int64_t sTime,
        COutPoint const& outpnt,
        CService const& addr,
        ...
        , outpoint{outpnt}
        , addr{addr}
        , pubKeyCollateralAddress{pkCollAddr}
        , pubKeyDynode{pkDN}
    {
    }
...
public:
    ...
    enum CollateralStatus { COLLATERAL_OK, COLLATERAL_UTXO_NOT_FOUND, COLLATERAL_INVALID_AMOUNT, COLLATERAL_INVALID_PUBKEY };
    ...
    {
        return nActiveStateIn == DYNODE_ENABLED || nActiveStateIn == DYNODE_PRE_ENABLED || nActiveStateIn == DYNODE_EXPIRED ||
    ...
    bool IsValidForPayment() const
    {
        ...
        if (!sporkManager.IsSporkActive(SPORK_14_REQUIRE_SENTINEL_FLAG) && (nActiveState == DYNODE_SENTINEL_PING_EXPIRED)) {
            return true;
        }
...
inline bool operator==(const CDynode& a, const CDynode& b) { return a.outpoint == b.outpoint; }
inline bool operator!=(const CDynode& a, const CDynode& b) { return !(a.outpoint == b.outpoint); }
...
public:
    ...
    CDynodeBroadcast(const CDynode& dn)
        : CDynode(dn)
        , fRecovery(false)
    {
    }
    CDynodeBroadcast(CService addrNew,
        COutPoint outpointNew,
        CPubKey pubKeyCollateralAddressNew,
        CPubKey pubKeyDynodeNew,
        int nProtocolVersionIn)
        : CDynode(addrNew, outpointNew, pubKeyCollateralAddressNew, pubKeyDynodeNew, nProtocolVersionIn)
        , fRecovery(false)
    {
    }
