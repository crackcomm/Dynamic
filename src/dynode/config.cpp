...
void CDynodeConfig::add(const std::string& alias,
    const std::string& ip,
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Dynode config file\n"
                                    ...
                                    "# Example: dn1 123.123.123.123:33300 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg "
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
            if (port != mainnetDefaultPort) {
                strErr = _("Invalid port detected in dynode.conf") + "\n" + tfm::format(_("Port: %d"), port) + "\n" +
                         tfm::format(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                         tfm::format(_("(must be %d for mainnet)"), mainnetDefaultPort);
                streamConfig.close();
                return false;
            }
        } else if (port == mainnetDefaultPort) {
            strErr = _("Invalid port detected in dynode.conf") + "\n" + tfm::format(_("Line: %d"), linenumber) + "\n\"" + line + "\"" +
                     "\n" + tfm::format(_("(%d could be used only on mainnet)"), mainnetDefaultPort);
            streamConfig.close();
            return false;
        }
