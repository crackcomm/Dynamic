...
bool CGovernanceObject::Sign(const CKey& keyDynode, const CPubKey& pubKeyDynode)
{
    ...
    LogPrint(BCLog::GOBJECT, "CGovernanceObject::Sign -- pubkey id = %s, dynode = %s\n",
...
bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingDynode, bool& fMissingConfirmations, bool fCheckCollateral) const
{
    ...
    case GOVERNANCE_OBJECT_PROPOSAL: {
        ...
        if (!validator.Validate(false)) {
            strError = tfm::format("Invalid proposal data, error messages: %s", validator.GetErrorMessages());
            return false;
        }
    ...
    default: {
        strError = tfm::format("Invalid object type %d", nObjectType);
        return false;
    }
    }
}
...
bool CGovernanceObject::IsCollateralValid(std::string& strError, bool& fMissingConfirmations) const
{
    ...
    if (!GetTransaction(nCollateralHash, txCollateral, Params().GetConsensus(), nBlockHash, true)) {
        strError = tfm::format("Can't find collateral tx %s", nCollateralHash.ToString());
        LogPrintf("CGovernanceObject::IsCollateralValid -- %s\n", strError);
        return false;
    }

    if (nBlockHash == uint256()) {
        strError = tfm::format("Collateral tx %s is not mined yet", txCollateral->ToString());
        LogPrintf("CGovernanceObject::IsCollateralValid -- %s\n", strError);
        return false;
    }

    if (txCollateral->vout.size() < 1) {
        strError = tfm::format("tx vout size less than 1 | %d", txCollateral->vout.size());
        LogPrintf("CGovernanceObject::IsCollateralValid -- %s\n", strError);
        return false;
    }
    ...
    for (const auto& output : txCollateral->vout) {
        ...
        if (!output.scriptPubKey.IsPayToPublicKeyHash() && !output.scriptPubKey.IsUnspendable()) {
            strError = tfm::format("Invalid Script %s", txCollateral->ToString());
            LogPrintf("CGovernanceObject::IsCollateralValid -- %s\n", strError);
            return false;
        }
    ...
    if (!foundOpReturn) {
        strError = tfm::format("Couldn't find opReturn %s in %s", nExpectedHash.ToString(), txCollateral->ToString());
        LogPrintf("CGovernanceObject::IsCollateralValid -- %s\n", strError);
        return false;
    }
    ...
    if (nConfirmationsIn < GOVERNANCE_FEE_CONFIRMATIONS) {
        strError = tfm::format("Collateral requires at least %d confirmations to be relayed throughout the network (it has only %d)", GOVERNANCE_FEE_CONFIRMATIONS, nConfirmationsIn);
