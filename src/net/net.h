...
enum threadId { THREAD_NTP, THREAD_MAX };
...
public:
    ...
    bool OpenNetworkConnection(const CAddress& addrConnect,
        ...
        bool fFeeler = false,
        bool fAddnode = false,
        bool fConnectToDynode = false);
    bool OpenDynodeConnection(const CAddress& addrConnect);
    bool CheckIncomingNonce(uint64_t nonce);

    struct CFullyConnectedOnly {
        bool operator()(const CNode* pnode) const { return NodeFullyConnected(pnode); }
    };
    ...
    {
        return ForNode(addr, cond, [](CNode* pnode) { return true; });
    }
