...
void ThreadMapPort()
{
    std::string port = tfm::format("%u", GetListenPort());
...
static std::string GetDNSHost(const CDNSSeedData& data, ServiceFlags* requiredServiceBits)
{
    ...
    return tfm::format("x%x.%s", *requiredServiceBits, data.host);
}
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (!addrBind.GetSockAddr((struct sockaddr*)&sockaddr, &len)) {
        strError = tfm::format("Error: Bind address family for %s not supported", addrBind.ToString());
        LogPrintf("%s\n", strError);
        return false;
    }
    ...
    if (hListenSocket == INVALID_SOCKET) {
        strError = tfm::format("Error: Couldn't open socket for incoming connections (socket returned error %s)", NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        return false;
    }
    ...
    if (!SetSocketNonBlocking(hListenSocket, true)) {
        strError = tfm::format("BindListenPort: Setting listening socket to non-blocking failed, error %s\n", NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        return false;
    }
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = tfm::format(_("Unable to bind to %s on this computer. %s is probably already running."), addrBind.ToString(), _(PACKAGE_NAME));
        else
            strError = tfm::format(_("Unable to bind to %s on this computer (bind returned error %s)"), addrBind.ToString(), NetworkErrorString(nErr));
        LogPrintf("%s\n", strError);
    ...
    if (listen(hListenSocket, SOMAXCONN) == SOCKET_ERROR) {
        strError = tfm::format(_("Error: Listening for incoming connections failed (listen returned error %s)"), NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        CloseSocket(hListenSocket);
        return false;
    }
