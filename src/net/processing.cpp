...
void PushNodeVersion(CNode* pnode, CConnman& connman, int64_t nTime)
{
    ...
    connman.PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION)
                                   .Make(NetMsgType::VERSION, PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe, nonce,
                                       strSubVersion, nNodeStartingHeight, ::fRelayTxes));

    if (fLogIPs)
        LogPrint(BCLog::NET, "send version message: version %d, blocks=%d, us=%s, them=%s, peer=%d\n", PROTOCOL_VERSION,
            nNodeStartingHeight, addrMe.ToString(), addrYou.ToString(), nodeid);
    else
        LogPrint(BCLog::NET, "send version message: version %d, blocks=%d, us=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight,
            addrMe.ToString(), nodeid);
}
...
void InitializeNode(CNode* pnode, CConnman& connman)
{
    ...
    NodeId nodeid = pnode->GetId();
    {
        ...
        mapNodeState.emplace_hint(
            mapNodeState.end(), std::piecewise_construct, std::forward_as_tuple(nodeid), std::forward_as_tuple(addr, std::move(addrName)));
    }
    if (!pnode->fInbound)
...
bool MarkBlockAsInFlight(NodeId nodeid,
    const uint256& hash,
    ...
    if (nodestate->fProvidesHeaderAndIDs) {
        ...
        connman.ForNode(nodeid, [&connman](CNode* pfrom) {
            ...
            if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
                ...
                connman.ForNode(
                    lNodesAnnouncingHeaderAndIDs.front(), [&connman, fAnnounceUsingCMPCTBLOCK, nCMPCTBLOCKVersion](CNode* pnodeStop) {
                        connman.PushMessage(pnodeStop, CNetMsgMaker(pnodeStop->GetSendVersion())
            ...
            connman.PushMessage(
                pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::SENDCMPCT, fAnnounceUsingCMPCTBLOCK, nCMPCTBLOCKVersion));
...
void PeerLogicValidation::BlockChecked(const CBlock& block, const CValidationState& state)
{
    ...
    if (state.IsInvalid(nDoS)) {
        if (it != mapBlockSource.end() && State(it->second.first)) {
            ...
            CBlockReject reject = {
                (unsigned char)state.GetRejectCode(), state.GetRejectReason().substr(0, MAX_REJECT_MESSAGE_LENGTH), hash};
        ...
        LogPrint(BCLog::NET, "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK) {
                ...
                if (mi != mapBlockIndex.end()) {
                    if (mi->second->nChainTx && !mi->second->IsValid(BLOCK_VALID_SCRIPTS) && mi->second->IsValid(BLOCK_VALID_TREE)) {
                    ...
                    if (chainActive.Contains(mi->second)) {
                        ...
                        send =
                            mi->second->IsValid(BLOCK_VALID_SCRIPTS) && (pindexBestHeader != NULL) &&
                ...
                if (send && connman.OutboundTargetReached(true) &&
                    (((pindexBestHeader != NULL) && (pindexBestHeader->GetBlockTime() - mi->second->GetBlockTime() > nOneWeek)) ||
                ...
                if (!push && inv.type == MSG_DYNODE_PAYMENT_VOTE) {
                    if (dnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        connman.PushMessage(
                            pfrom, msgMaker.Make(NetMsgType::DYNODEPAYMENTVOTE, dnpayments.mapDynodePaymentVotes[inv.hash]));
                        push = true;
                    }
                }
                        ...
                        BOOST_FOREACH (CDynodePayee& payee, dnpayments.mapDynodeBlocks[mi->second->nHeight].vecPayees) {
                            ...
                            BOOST_FOREACH (uint256& hash, vecVoteHashes) {
                                if (dnpayments.HasVerifiedPaymentVote(hash)) {
                                    connman.PushMessage(
                                        pfrom, msgMaker.Make(NetMsgType::DYNODEPAYMENTVOTE, dnpayments.mapDynodePaymentVotes[hash]));
                                }
                            }
                        }
                        push = true;
    ...
    const std::atomic<bool>& interruptMsgProc)
{
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION) {
            ...
            connman.PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION)
                                           .Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                                               tfm::format("Version must be %d or greater", MIN_PEER_PROTO_VERSION)));
            pfrom->fDisconnect = true;
            return false;
        }
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        for (; pindex; pindex = chainActive.Next(pindex)) {
            ...
            if (fPruneMode &&
                (!(pindex->nStatus & BLOCK_HAVE_DATA) || pindex->nHeight <= chainActive.Tip()->nHeight - nPrunedBlocksLikelyToHave)) {
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
...

        {
            ...
            std::map<uint256, std::pair<NodeId, std::list<QueuedBlock>::iterator> >::iterator blockInFlightIt =
        ...
        const CBlockIndex* pindexLast = NULL;
        {
            ...
            if (nCount > 0 && mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
                ...
                LogPrint(BCLog::NET,
                    ...
                    headers[0].GetHash().ToString(), headers[0].hashPrevBlock.ToString(), pindexBestHeader->nHeight, pfrom->id,
                    nodestate->nUnconnectingHeaders);
...

        {
            ...
            if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
                ...
                if (!chainActive.Contains(pindexWalk)) {
                    ...
                    if (vGetData.size() > 0) {
                        if (nodestate->fSupportsDesiredCmpctVersion && vGetData.size() == 1 && mapBlocksInFlight.size() == 1 &&
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert(chainparams.AlertKey())) {
                ...
                {
                    connman.ForEachNode([&alert, &connman](CNode* pnode) { alert.RelayTo(pnode, connman); });
                }
            } else {
    ...
    BOOST_FOREACH (const CBlockReject& reject, state.rejects) {
        connman.PushMessage(pnode,
            CNetMsgMaker(INIT_PROTO_VERSION)
                .Make(NetMsgType::REJECT, (std::string)NetMsgType::BLOCK, reject.chRejectCode, reject.strRejectReason, reject.hashBlock));
    }
    state.rejects.clear();
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    try {
        ...
        connman.PushMessage(pfrom,
            CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_MALFORMED, std::string("error parsing message")));
...
public:
    CompareInvMempoolOrder(CTxMemPool* mempool) { mp = mempool; }
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !pto->fDisconnect && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - nMaxTipAge) {
                state.fSyncStarted = true;
                state.nHeadersSyncTimeout = GetTimeMicros() + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                                            HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime()) /
        ...
        //
        {
            ...
            bool fRevertToInv = ((!state.fPreferHeaders && (!state.fPreferHeaderAndIDs || pto->vBlockHashesToAnnounce.size() > 1)) ||
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    if (vHeaders.size() > 1) {
                        ...
                        LogPrint(
                            BCLog::NET, "%s: sending header %s to peer=%d\n", __func__, vHeaders.front().GetHash().ToString(), pto->id);
                    }
        ...
        if (!pto->fDisconnect && !pto->fClient && (fFetch || !IsInitialBlockDownload()) &&
            ...
            FindNextBlocksToDownload(
                pto->GetId(), MAX_BLOCKS_IN_TRANSIT_PER_PEER - state.nBlocksInFlight, vToDownload, staller, consensusParams);
