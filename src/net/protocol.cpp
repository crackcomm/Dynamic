...
CInv::CInv(const std::string& strType, const uint256& hashIn)
{
    ...
    if (i == ARRAYLEN(ppszTypeName))
        throw std::out_of_range(tfm::format("CInv::CInv(string, uint256): unknown type '%s'", strType));
    hash = hashIn;
}
...
{
    try {
        return tfm::format("%s %s", GetCommand(), hash.ToString());
    } catch (const std::out_of_range&) {
        return tfm::format("0x%08x %s", type, hash.ToString());
    }
}
