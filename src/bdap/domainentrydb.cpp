...
bool CDomainEntryDB::AddDomainEntry(const CDomainEntry& entry, const int op)
{
    ...
    {
        LOCK(cs_bdap_entry);
        writeState = Write(make_pair(std::string("dc"), entry.GetFullObjectPath()), entry) && Write(make_pair(std::string("txid"), entry.txHash), entry.GetFullObjectPath());
    }
    if (writeState)
...
bool CDomainEntryDB::UpdateDomainEntry(const std::vector<unsigned char>& vchObjectPath, const CDomainEntry& entry)
{
    ...
    bool writeState = false;
    writeState = Update(make_pair(std::string("dc"), entry.GetFullObjectPath()), entry) && Write(make_pair(std::string("txid"), entry.txHash), entry.GetFullObjectPath());
    if (writeState)
...
bool CheckNewDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
...
bool CheckDeleteDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
...
bool CheckUpdateDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
    //if exists, check for owner's signature
    if (!CommonDataCheck(entry, vvchOpParameters, errorMessage))
        return error(errorMessage.c_str());
...
bool CheckMoveDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
...
bool CheckExecuteDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
    //check name in operation matches entry data in leveldb
    //check if exists already
    //if exists, check for owner's signature
...
bool CheckBindDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
...
bool CheckRevokeDomainEntryTxInputs(const CDomainEntry& entry, const CScript& scriptOp, const vchCharString& vvchOpParameters, std::string& errorMessage, bool fJustCheck)
{
    //check name in operation matches entry data in leveldb
    //check if names exists already
...
bool CheckDomainEntryTxInputs(const CCoinsViewCache& inputs, const CTransactionRef& tx, int op, const std::vector<std::vector<unsigned char> >& vvchArgs, bool fJustCheck, int nHeight, std::string& errorMessage, bool bSanityCheck)
{
