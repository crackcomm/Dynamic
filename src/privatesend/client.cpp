...
void CPrivateSendClientManager::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::PSQUEUE) {
        if (pfrom->nVersion < MIN_PRIVATESEND_PEER_PROTO_VERSION) {
            LogPrint(BCLog::PRIVATESEND, "PSQUEUE -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, tfm::format("Version must be %d or greater", MIN_PRIVATESEND_PEER_PROTO_VERSION)));
            return;
        }
        ...
        if (psq.fReady) {
            ...
            LogPrint(BCLog::PRIVATESEND, "PSQUEUE -- nLastPsq: %d  threshold: %d  nPsqCount: %d\n", infoDn.nLastPsq, nThreshold, dnodeman.nPsqCount);
...
void CPrivateSendClientSession::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::PSSTATUSUPDATE) {
        if (pfrom->nVersion < MIN_PRIVATESEND_PEER_PROTO_VERSION) {
            LogPrint(BCLog::PRIVATESEND, "PSSTATUSUPDATE -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, tfm::format("Version must be %d or greater", MIN_PRIVATESEND_PEER_PROTO_VERSION)));
            return;
        }
        ...
        LogPrint(BCLog::PRIVATESEND, "PSSTATUSUPDATE -- nMsgSessionID %d  nMsgState: %d  nEntriesCount: %d  nMsgStatusUpdate: %d  nMsgMessageID %d (%s)\n",
        ...
        if (pfrom->nVersion < MIN_PRIVATESEND_PEER_PROTO_VERSION) {
            LogPrint(BCLog::PRIVATESEND, "PSFINALTX -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, tfm::format("Version must be %d or greater", MIN_PRIVATESEND_PEER_PROTO_VERSION)));
            return;
        }
        ...
        if (pfrom->nVersion < MIN_PRIVATESEND_PEER_PROTO_VERSION) {
            LogPrint(BCLog::PRIVATESEND, "PSCOMPLETE -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, tfm::format("Version must be %d or greater", MIN_PRIVATESEND_PEER_PROTO_VERSION)));
            return;
        }
        ...
        if (!infoMixingDynode.fInfoValid)
            return;
        if (infoMixingDynode.addr != pfrom->addr) {
            LogPrint(BCLog::PRIVATESEND, "PSCOMPLETE -- message doesn't match current Dynode: infoMixingDynode=%s  addr=%s\n", infoMixingDynode.addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CPrivateSendClientSession::GetStatus(bool fWaitForBlock)
{
    ...
    case POOL_STATE_QUEUE:
        if (nStatusMessageProgress % 70 <= 30)
            ...
            strSuffix = "...";
        return tfm::format(_("Submitted to dynode, waiting in queue %s"), strSuffix);
        ;
    case POOL_STATE_ACCEPTING_ENTRIES:
        if (nEntriesCount == 0) {
            ...
            if (nStatusMessageProgress % 70 <= 40)
                return tfm::format(_("Submitted following entries to dynode: %u / %d"), nEntriesCount, CPrivateSend::GetMaxPoolTransactions());
            else if (nStatusMessageProgress % 70 <= 50)
                strSuffix = ".";
                ...
                strSuffix = "...";
            return tfm::format(_("Submitted to dynode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, CPrivateSend::GetMaxPoolTransactions(), strSuffix);
        }
    case POOL_STATE_SIGNING:
        if (nStatusMessageProgress % 70 <= 40)
            ...
            strSuffix = "...";
        return tfm::format(_("Found enough users, signing ( waiting %s )"), strSuffix);
    ...
    default:
        return tfm::format(_("Unknown state: id = %u"), nState);
    }
}
...
bool CPrivateSendClientSession::SendDenominate(const std::vector<CTxPSIn>& vecTxPSIn, const std::vector<CTxOut>& vecTxOut, CConnman& connman)
{
...

    {
        ...
        for (const auto& txdsin : vecTxPSIn) {
            LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::SendDenominate -- txdsin=%s\n", txdsin.ToString());
            tx.vin.push_back(txdsin);
        }

        for (const CTxOut& txout : vecTxOut) {
            LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::SendDenominate -- txout=%s\n", txout.ToString());
...
bool CPrivateSendClientManager::CheckAutomaticBackup()
{
    ...
    if (pwalletMain->nKeysLeftSinceAutoBackup < PRIVATESEND_KEYS_THRESHOLD_STOP) {
        ...
        strAutoDenomResult = tfm::format(_("Very low number of keys left: %d") + ", " + _("no mixing available."), pwalletMain->nKeysLeftSinceAutoBackup);
        ...
        strAutoDenomResult = tfm::format(_("Very low number of keys left: %d"), pwalletMain->nKeysLeftSinceAutoBackup);
        ...
        if (fCreateAutoBackups) {
            ...
            if (!AutoBackupWallet(pwalletMain, "", warningString, errorString)) {
                ...
                if (!errorString.empty()) {
                    ...
                    strAutoDenomResult = tfm::format(_("ERROR! Failed to create automatic backup") + ": %s", errorString);
                    return false;
                }
            }
        } else {
...
bool CPrivateSendClientSession::DoAutomaticDenominating(CConnman& connman, bool fDryRun)
{
    ...
    CAmount nValueMin = CPrivateSend::GetSmallestDenomination();
    {
        ...
        if (dnodeman.size() == 0) {
            LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::DoAutomaticDenominating -- No Dynodes detected\n");
        ...
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::DoAutomaticDenominating -- nValueMin: %f, nBalanceNeedsAnonymized: %f, nBalanceAnonimizableNonDenom: %f, nBalanceDenominatedConf: %f, nBalanceDenominatedUnconf: %f, nBalanceDenominated: %f\n",
...
bool CPrivateSendClientSession::ProcessPendingPsaRequest(CConnman& connman)
{
    ...
    bool fDone = connman.ForNode(pendingPsaRequest.GetAddr(), [&](CNode* pnode) {
        LogPrint(BCLog::PRIVATESEND, "-- processing psa queue for addr=%s\n", pnode->addr.ToString());
        nTimeLastSuccessfulStep = GetTime();
...
bool CPrivateSendClientSession::MakeCollateralAmounts(CConnman& connman)
{
    ...
    if (!pwalletMain->SelectCoinsGrouppedByAddresses(vecTally, false, false)) {
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::MakeCollateralAmounts -- SelectCoinsGrouppedByAddresses can't find any inputs!\n");
        return false;
    }
...
bool CPrivateSendClientSession::CreateDenominated(CConnman& connman)
{
    ...
    if (!pwalletMain->SelectCoinsGrouppedByAddresses(vecTally)) {
        LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::CreateDenominated -- SelectCoinsGrouppedByAddresses can't find any inputs!\n");
        return false;
    }
        ...
        for (auto it = vecStandardDenoms.rbegin(); it != vecStandardDenoms.rend(); ++it) {
            ...
            if (fSkip) {
                ...
                if (privateSendClient.IsDenomSkipped(nDenomValue)) {
                    strAutoDenomResult = tfm::format(_("Too many %f denominations, skipping."), (float)nDenomValue / COIN);
                ...
                if (nDenomValue != vecStandardDenoms.front() && pwalletMain->CountInputsWithAmount(nDenomValue) > DENOMS_COUNT_MAX) {
                    strAutoDenomResult = tfm::format(_("Too many %f denominations, removing."), (float)nDenomValue / COIN);
                    LogPrintf("CPrivateSendClientSession::CreateDenominated -- %s\n", strAutoDenomResult);
...
void CPrivateSendClientManager::UpdatedBlockTip(const CBlockIndex* pindex)
{
    ...
    LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::UpdatedBlockTip -- nCachedBlockHeight: %d\n", nCachedBlockHeight);
}
