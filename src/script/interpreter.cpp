...
inline bool
set_error(ScriptError* ret, const ScriptError serror)
{
    if (ret)
        *ret = serror;
    return false;
...
static inline void
popstack(std::vector<valtype>& stack)
{
    if (stack.empty())
...
bool static CheckMinimalPush(const valtype& data, opcodetype opcode)
{
    ...
    if (data.size() == 0) {
        ...
        // Must have used a direct push (opcode indicating number of bytes pushed +
        // those bytes).
        ...
        while (pc < pend) {
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
                ...
                case OP_CODESEPARATOR: {
                    // If SCRIPT_VERIFY_CONST_SCRIPTCODE flag is set, use of
                    // OP_CODESEPARATOR is rejected in pre-segwit script, even in an
                    // unexecuted branch (this is checked above the opcode case
                    // statement).
                ...
                case OP_CHECKMULTISIGVERIFY: {
                    ...
                    while (fSuccess && nSigsCount > 0) {
                        ...
                        // evaluation distinguishable by CHECKMULTISIG NOT if the STRICTENC
                        // flag is set. See the script_(in)valid tests for details.
                        if (!CheckSignatureEncoding(vchSig, flags, serror) ||
                            !CheckPubKeyEncoding(vchPubKey, flags, sigversion, serror)) {
                            // serror is set
                            return false;
...
public:
    ...
    template <typename S>
    void
    SerializeInput(S& s, unsigned int nInput) const
    {
    ...
    template <typename S>
    void
    SerializeOutput(S& s, unsigned int nOutput) const
    {
...
template <class T>
uint256
SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (flags & SCRIPT_VERIFY_P2SH)
        stackCopy = stack;
    if (!EvalScript(stack, scriptPubKey, flags, checker, SigVersion::BASE,
            serror))
        // serror is set
        return false;
    if (stack.empty())
