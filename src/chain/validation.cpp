    ...
    for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
        ...
        if (txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG) {
            ...
            nMinTime = std::max(nMinTime,
                nCoinTime + (int64_t)((txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_MASK) << CTxIn::SEQUENCE_LOCKTIME_GRANULARITY) - 1);
        } else {
...
{
    return tfm::format("%s%s (code %i)", state.GetRejectReason(), state.GetDebugMessage().empty() ? "" : ", " + state.GetDebugMessage(),
        state.GetRejectCode());
}
...
bool ValidateBDAPInputs(const CTransactionRef& tx,
    ...
    bool fDryRun)
{
    ...
    if (fRequireStandard && tx.nVersion >= 2 && tx.nVersion != BDAP_TX_VERSION &&
    ...
    if (instantsend.HasTxLockRequest(hash) && !CTxLockRequest(tx).IsValid())
        return state.DoS(
            10, error("AcceptToMemoryPool : CTxLockRequest %s is invalid", hash.ToString()), REJECT_INVALID, "bad-txlockrequest");
    ...
    BOOST_FOREACH (const CTxIn& txin, tx.vin) {
        ...
        if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hash != hashLocked)
            return state.DoS(10,
                error("AcceptToMemoryPool : Transaction %s conflicts with completed Transaction Lock %s", hash.ToString(),
                    hashLocked.ToString()),
            ...
            if (itConflicting != pool.mapNextTx.end()) {
                ...
                if (!setConflicts.count(ptxConflicting->GetHash())) {
                    ...
                    if (instantsend.HasTxLockRequest(ptxConflicting->GetHash())) {
                        ...
                        return state.DoS(0,
                            error("AcceptToMemoryPool : Transaction %s conflicts with Transaction Lock Request %s", hash.ToString(),
                        ...
                        return state.DoS(0,
                            error("AcceptToMemoryPool : Transaction Lock Request %s conflicts with transaction %s", hash.ToString(),
                                ptxConflicting->GetHash().ToString()),
...

    {
        ...
        CTxMemPoolEntry entry(ptx, nFees, nAcceptTime, dPriority, chainActive.Height(), inChainInputValue, fSpendsCoinbase, nSigOps, lp);
        ...
        if ((nSigOps > MAX_STANDARD_TX_SIGOPS) || (nBytesPerSigOp && nSigOps > nSize / nBytesPerSigOp))
            return state.DoS(0, false, REJECT_NONSTANDARD, "bad-txns-too-many-sigops", false, tfm::format("%d", nSigOps));

        CAmount mempoolRejectFee = pool.GetMinFee(GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000).GetFee(nSize);
        if (mempoolRejectFee > 0 && nModifiedFees < mempoolRejectFee) {
            return state.DoS(
                0, false, REJECT_INSUFFICIENTFEE, "mempool min fee not met", false, tfm::format("%d < %d", nFees, mempoolRejectFee));
        ...
        if (nAbsurdFee && nFees > nAbsurdFee)
            return state.Invalid(false, REJECT_HIGHFEE, "absurdly-high-fee", tfm::format("%d > %d", nFees, nAbsurdFee));
        ...
        if (!pool.CalculateMemPoolAncestors(
                entry, setAncestors, nLimitAncestors, nLimitAncestorSize, nLimitDescendants, nLimitDescendantSize, errString)) {
        ...
        BOOST_FOREACH (CTxMemPool::txiter ancestorIt, setAncestors) {
            ...
            if (setConflicts.count(hashAncestor)) {
                return state.DoS(10, false, REJECT_INVALID, "bad-txns-spends-conflicting-tx", false,
                    tfm::format("%s spends conflicting transaction %s", hash.ToString(), hashAncestor.ToString()));
            }
        }
        ...
        if (fReplacementTransaction) {
            ...
            BOOST_FOREACH (const uint256& hashConflicting, setConflicts) {
                ...
                if (newFeeRate <= oldFeeRate) {
                    return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
                        tfm::format("rejecting replacement %s; new feerate %s <= old feerate %s", hash.ToString(), newFeeRate.ToString(),
                            oldFeeRate.ToString()));
                }
            ...
            if (nConflictingCount <= maxDescendantsToVisit) {
                ...
                return state.DoS(0, false, REJECT_NONSTANDARD, "too many potential replacements", false,
                    tfm::format("rejecting replacement %s; too many potential replacements (%d > %d)\n", hash.ToString(), nConflictingCount,
            ...
            for (unsigned int j = 0; j < tx.vin.size(); j++) {
                ...
                if (!setConflictsParents.count(tx.vin[j].prevout.hash)) {
                    ...
                    if (pool.mapTx.find(tx.vin[j].prevout.hash) != pool.mapTx.end())
                        return state.DoS(0, false, REJECT_NONSTANDARD, "replacement-adds-unconfirmed", false,
                            tfm::format("replacement %s adds unconfirmed input, idx %d", hash.ToString(), j));
                }
            }
            ...
            if (nModifiedFees < nConflictingFees) {
                return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
                    tfm::format("rejecting replacement %s, less fees than conflicting txs; %s < %s", hash.ToString(),
                        FormatMoney(nModifiedFees), FormatMoney(nConflictingFees)));
            }
            ...
            if (nDeltaFees < ::incrementalRelayFee.GetFee(nSize)) {
                return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
                    tfm::format("rejecting replacement %s, not enough additional fees to relay; %s < %s", hash.ToString(),
                        FormatMoney(nDeltaFees), FormatMoney(::incrementalRelayFee.GetFee(nSize))));
            }
        }
        ...
        if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true)) {
            return error("%s: BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not STANDARD flags %s, %s", __func__,
        ...
        BOOST_FOREACH (const CTxMemPool::txiter it, allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s %s additional fees, %d delta bytes\n",
                it->GetTx().GetHash().ToString(), hash.ToString(), FormatMoney(nModifiedFees - nConflictingFees), CURRENCY_UNIT,
...
{
    std::vector<COutPoint> coins_to_uncache;
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit,
    ...
    bool fDryRun)
{
    return AcceptToMemoryPoolWithTime(
        pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, fDryRun);
}
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip ||
        (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 6))) {
        ...
        if (pindexBestForkTip && pindexBestForkBase) {
            ...
            if (pindexBestInvalid->nHeight > chainActive.Height() + 10)
                ...
                LogPrintf("%s: Warning: Found invalid chain which has higher work (at least ~10 blocks worth of work) than our best "
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("%s:  current best=%s  height=%d  log2_work=%.8g  date=%s\n", __func__, tip->GetBlockHash().ToString(), chainActive.Height(),
        log(tip->nChainWork.getdouble()) / log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S", tip->GetBlockTime()));
    CheckForkWarningConditions();
}
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        if (coin.IsCoinBase()) {
            if (nSpendHeight - coin.nHeight < COINBASE_MATURITY)
                return state.Invalid(false, REJECT_INVALID, "bad-txns-premature-spend-of-coinbase",
                    tfm::format("tried to spend coinbase at depth %d", nSpendHeight - coin.nHeight));
        }
    ...
    if (nValueIn < tx.GetValueOut())
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-in-belowout", false,
            tfm::format("value in (%s) < value out (%s)", FormatMoney(nValueIn), FormatMoney(tx.GetValueOut())));
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        if (check2())
                            return state.Invalid(false, REJECT_NONSTANDARD,
                                tfm::format("non-mandatory-script-verify-flag (%s)", ScriptErrorString(check.GetScriptError())));
                    }
                    ...
                    return state.DoS(100, false, REJECT_INVALID,
                        tfm::format("mandatory-script-verify-flag-failed (%s)", ScriptErrorString(check.GetScriptError())));
                }
            }
        }
    }
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
                    ...
                    addressIndex.push_back(
                        std::make_pair(CAddressIndexKey(2, uint160(hashBytes), pindex->nHeight, i, hash, k, false), out.nValue));
                    ...
                    // undo receiving activity
                    addressIndex.push_back(
                        std::make_pair(CAddressIndexKey(1, uint160(hashBytes), pindex->nHeight, i, hash, k, false), out.nValue));
                ...
                if (fAddressIndex) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
                        ...
                        addressIndex.push_back(std::make_pair(
                            CAddressIndexKey(2, uint160(hashBytes), pindex->nHeight, i, hash, j, true), prevout.nValue * -1));
                        // restore unspent index
                        addressUnspentIndex.push_back(
                            std::make_pair(CAddressUnspentKey(2, uint160(hashBytes), input.prevout.hash, input.prevout.n),
                                CAddressUnspentValue(prevout.nValue, scriptPubKey, undoHeight)));
                        ...
                        // undo spending activity
                        addressIndex.push_back(std::make_pair(
                            CAddressIndexKey(1, uint160(hashBytes), pindex->nHeight, i, hash, j, true), prevout.nValue * -1));
                        // restore unspent index
                        addressUnspentIndex.push_back(
...
public:
    ...
    {
        return ((pindex->nVersion & VERSIONBITS_TOP_MASK) == VERSIONBITS_TOP_BITS) && ((pindex->nVersion >> bit) & 1) != 0 &&
...
static bool ConnectBlock(const CBlock& block,
    CValidationState& state,
    CBlockIndex* pindex,
    ...
    bool fJustCheck = false)
{
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != mapBlockIndex.end()) {
            if (it->second->GetAncestor(pindex->nHeight) == pindex && pindexBestHeader->GetAncestor(pindex->nHeight) == pindex &&
                ...
                fScriptChecks = (GetBlockProofEquivalentTime(*pindexBestHeader, *pindex, *pindexBestHeader, chainparams.GetConsensus()) <=
                                 60 * 60 * 24 * 7 * 2);
            }
        }
    }
    ...
    if (pindex->nHeight == chainparams.GetConsensus().nSuperblockStartBlock &&
        chainparams.GetConsensus().nSuperblockStartHash != uint256() && block.GetHash() != chainparams.GetConsensus().nSuperblockStartHash)
        return state.DoS(100, error("ConnectBlock(): invalid superblock start"), REJECT_INVALID, "bad-sb-start");
        ...
        if (!tx.IsCoinBase()) {
            ...
            if (fAddressIndex || fSpentIndex) {
                for (size_t j = 0; j < tx.vin.size(); j++) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
                        hashBytes =
                            uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin() + 2, prevout.scriptPubKey.begin() + 22));
                        addressType = 2;
                    } else if (prevout.scriptPubKey.IsPayToPublicKeyHash()) {
                        hashBytes =
                            uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin() + 3, prevout.scriptPubKey.begin() + 23));
                        addressType = 1;
                    } else {
                        hashBytes.SetNull();
                        addressType = 0;
                    ...
                    if (fAddressIndex && addressType > 0) {
                        ...
                        addressIndex.push_back(std::make_pair(
                            CAddressIndexKey(addressType, hashBytes, pindex->nHeight, i, txhash, j, true), prevout.nValue * -1));

                        // remove address from unspent index
                        addressUnspentIndex.push_back(std::make_pair(
                            CAddressUnspentKey(addressType, hashBytes, input.prevout.hash, input.prevout.n), CAddressUnspentValue()));
                    }
        ...
        if (fAddressIndex) {
            for (unsigned int k = 0; k < tx.vout.size(); k++) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
                    ...
                    addressIndex.push_back(
                        std::make_pair(CAddressIndexKey(2, uint160(hashBytes), pindex->nHeight, i, txhash, k, false), out.nValue));
                    // record unspent output
                    addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(2, uint160(hashBytes), txhash, k),
                        CAddressUnspentValue(out.nValue, scriptPubKey, pindex->nHeight)));
                } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
                    // Remove BDAP portion of the script
                    CScript scriptPubKey;
                    CScript scriptPubKeyOut;
                    ...
                    // record receiving activity
                    addressIndex.push_back(
                        std::make_pair(CAddressIndexKey(1, uint160(hashBytes), pindex->nHeight, i, txhash, k, false), out.nValue));
                    // record unspent output
                    addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(1, uint160(hashBytes), txhash, k),
                        CAddressUnspentValue(out.nValue, scriptPubKey, pindex->nHeight)));
                } else {
                    continue;
                }
    ...
    std::string strError = "";
    {
        ...
        if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, nExpectedBlockValue)) {
            ...
            return state.DoS(0, error("ConnectBlock(DYN): couldn't find Dynode or Superblock payments"), REJECT_INVALID, "bad-cb-payee");
        }
    }
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        const CTransaction& tx = *block.vtx[i];
            ...
            if (OpCode == OP_REWARD_DYNODE) {
                ...
                if (CheckFluidDynodeDB()) {
                    if (!CheckSignatureQuorum(fluidDynode.FluidScript, strError)) {
                        return state.DoS(
                            0, error("ConnectBlock(DYN): %s", strError), REJECT_INVALID, "invalid-fluid-dynode-address-signature");
                    }
                ...
                if (CheckFluidMiningDB()) {
                    if (!CheckSignatureQuorum(fluidMining.FluidScript, strError)) {
                        return state.DoS(
                            0, error("ConnectBlock(DYN): %s", strError), REJECT_INVALID, "invalid-fluid-mining-address-signature");
                    }
                ...
                if (CheckFluidMintDB()) {
                    if (!CheckSignatureQuorum(fluidMint.FluidScript, strError)) {
                        return state.DoS(
                            0, error("ConnectBlock(DYN): %s", strError), REJECT_INVALID, "invalid-fluid-mint-address-signature");
                    }
...
void static UpdateTip(CBlockIndex* pindexNew, const CChainParams& chainParams)
{
    ...
    if (!IsInitialBlockDownload()) {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == THRESHOLD_ACTIVE || state == THRESHOLD_LOCKED_IN) {
                if (state == THRESHOLD_ACTIVE) {
                    std::string strWarning = tfm::format(_("Warning: unknown new rules activated (versionbit %i)"), bit);
                    ...
                    warningMessages.push_back(tfm::format("unknown new rules are about to activate (versionbit %i)", bit));
                }
            }
        }
        ...
        if (nUpgraded > 0)
            warningMessages.push_back(tfm::format("%d of last 100 blocks have unexpected version", nUpgraded));
        if (nUpgraded > 100 / 2) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
        ...
        GuessVerificationProgress(chainParams.TxData(), chainActive.Tip()), pcoinsTip->DynamicMemoryUsage() * (1.0 / (1 << 20)),
...
/** Disconnect chainActive's tip. You probably want to call mempool.removeForReorg and manually re-limit mempool size after this, with
 * cs_main held. */
    ...
    ConnectTrace& connectTrace)
{
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        BOOST_REVERSE_FOREACH (CBlockIndex* pindexConnect, vpindexToConnect) {
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(),
                    connectTrace)) {
                if (state.IsInvalid()) {
    ...
    if (fBlocksDisconnected) {
        ...
        LimitMempoolSize(
            mempool, GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000, GetArg("-mempoolexpiry", DEFAULT_MEMPOOL_EXPIRY) * 60 * 60);
    }
 ...
 */
bool ActivateBestChain(CValidationState& state, const CChainParams& chainparams, const std::shared_ptr<const CBlock>& pblock)
{
...
bool ReceivedBlockTransactions(const CBlock& block, CValidationState& state, CBlockIndex* pindexNew, const CDiskBlockPos& pos)
{
    ...
    if (pindexNew->pprev == NULL || pindexNew->pprev->nChainTx) {
        ...
        while (!queue.empty()) {
            ...
            std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator, std::multimap<CBlockIndex*, CBlockIndex*>::iterator> range =
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0,
                        error("CheckBlock(DYN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(),
                            hashLocked.ToString()),
    ...
    for (const auto& tx : block.vtx) {
        if (!CheckTransaction(*tx, state, false))
            return state.Invalid(false, state.GetRejectCode(), state.GetRejectReason(),
                tfm::format("Transaction check failed (tx hash %s) %s", tx->GetHash().ToString(), state.GetDebugMessage()));
        ...
        for (const auto& txout : tx->vout) {
            if (IsTransactionFluid(txout.scriptPubKey)) {
                ...
                if (!fluid.CheckFluidOperationScript(txout.scriptPubKey, block.nTime, strErrorMessage)) {
                    return error(
                        "CheckBlock(): %s, Block %s failed with %s", strErrorMessage, tx->GetHash().ToString(), FormatStateMessage(state));
                }
            }
        }
...
void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint(BCLog::PRUNE, "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d blk/rev pairs\n",
        nPruneTarget / 1024 / 1024, nCurrentUsage / 1024 / 1024, ((int64_t)nPruneTarget - (int64_t)nCurrentUsage) / 1024 / 1024,
        nLastBlockWeCanPrune, count);
}
...
{
    return GetDataDir() / "blocks" / tfm::format("%s%05u.dat", prefix, pos.nFile);
}
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        boost::this_thread::interruption_point();
        int percentageDone = std::max(1,
            std::min(99, (int)(((double)(chainActive.Height() - pindex->nHeight)) / (double)nCheckDepth * (nCheckLevel >= 4 ? 50 : 100))));
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            uiInterface.ShowProgress(_("Verifying blocks..."),
                std::max(1, std::min(99, 100 - (int)(((double)(chainActive.Height() - pindex->nHeight)) / (double)nCheckDepth * 50))));
            ...
            if (!ConnectBlock(block, state, pindex, coins, chainparams))
                return error(
                    "VerifyDB(): *** found unconnectable block at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
        }
    }

    LogPrintf("[DONE].\n");
    LogPrintf("No coin database inconsistencies in last %i blocks (%i transactions)\n", chainActive.Height() - pindexState->nHeight,
        nGoodTransactions);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    LogPrint(
                        BCLog::REINDEX, "Block Import: already had block %s at height %d\n", hash.ToString(), mapBlockIndex[hash]->nHeight);
                }
                ...
                while (!queue.empty()) {
                    ...
                    queue.pop_front();
                    std::pair<std::multimap<uint256, CDiskBlockPos>::iterator, std::multimap<uint256, CDiskBlockPos>::iterator> range =
...
void static CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator, std::multimap<CBlockIndex*, CBlockIndex*>::iterator> rangeGenesis =
    ...
    while (pindex != NULL) {
        ...
        // All parents having had data (at some point) is equivalent to all parents being VALID_TRANSACTIONS, which is equivalent to
        // nChainTx being set.
        ...
        } else { // If this block sorts worse than the current tip or some ancestor's block has never been seen, it cannot be in
        ...
        if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) && pindexFirstNeverProcessed == NULL && pindexFirstMissing != NULL) {
            // We HAVE_DATA for this block, have received data for all parents at some point, but we're currently missing data for some
            // parent.
        ...
        while (pindex) {
            ...
            std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator, std::multimap<CBlockIndex*, CBlockIndex*>::iterator> rangePar =
...
{
    return tfm::format("CBlockFileInfo(blocks=%u, size=%u, heights=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst, nHeightLast,
        DateTimeStrFormat("%Y-%m-%d", nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
}
