    ...
    while (it != script.end()) {
        ...
        if (script.GetOp2(it, op, &vch)) {
            if (op == OP_0) {
                ...
                ret += tfm::format("%i ", op - OP_1NEGATE - 1);
                continue;
            ...
            if (vch.size() > 0) {
                ret += tfm::format("0x%x 0x%x ", HexStr(it2, it - vch.size()), HexStr(it - vch.size(), it));
            } else {
                ret += tfm::format("0x%x ", HexStr(it2, it));
            }
            continue;
        }
        ret += tfm::format("0x%x ", HexStr(it2, script.end()));
        break;
    }
    ...
    while (pc < script.end()) {
        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4) {
            if (vch.size() <= static_cast<std::vector<unsigned char>::size_type>(4)) {
                str += tfm::format("%d", CScriptNum(vch, false).getint());
            } else {
