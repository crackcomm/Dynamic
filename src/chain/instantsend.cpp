        ...
        if (pfrom->nVersion < MIN_INSTANTSEND_PROTO_VERSION) {
            ...
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion())
                                           .Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                                               tfm::format("Version must be %d or greater", MIN_INSTANTSEND_PROTO_VERSION)));
            return;
        }
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest, CConnman& connman)
{
    ...
    for (const auto& txin : txLockRequest.tx->vin) {
        ...
        if (it != mapLockedOutpoints.end() && it->second != txLockRequest.GetHash()) {
            ...
            LogPrintf("CInstantSend::ProcessTxLockRequest -- WARNING: Found conflicting completed Transaction Lock, txid=%s, completed "
                      "lock txid=%s\n",
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    for (auto& outpointLockPair : txLockCandidate.mapOutPointLocks) {
        ...
        if (!dnodeman.GetDynodeRank(activeDynode.outpoint, nRank, nLockInputHeight, nMinRequiredProtocol)) {
            LogPrint(
                BCLog::INSTANTSEND, "CInstantSend::Vote -- Can't calculate rank for dynode %s\n", activeDynode.outpoint.ToStringShort());
            continue;
        }
        ...
        if (outpointLockPair.second.AddVote(vote)) {
            LogPrintf("CInstantSend::Vote -- Vote created successfully, relaying: txHash=%s, outpoint=%s, vote=%s\n", txHash.ToString(),
                outpointLockPair.first.ToStringShort(), nVoteHash.ToString());
...
bool CInstantSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if (it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint(BCLog::INSTANTSEND, "CInstantSend::%s -- Orphan vote: txid=%s  dynode=%s %s\n", __func__, txHash.ToString(),
            vote.GetDynodeOutpoint().ToStringShort(), fInserted ? "new" : "seen");
        ...
        if (itDnOV == mapDynodeOrphanVotes.end()) {
            ...
            if (itDnOV->second > GetTime() && itDnOV->second > GetAverageDynodeOrphanVoteTime()) {
                LogPrint(BCLog::INSTANTSEND, "CInstantSend::%s -- dynode is spamming orphan Transaction Lock Votes: txid=%s  dynode=%s\n",
    ...
    LogPrint(BCLog::INSTANTSEND, "CInstantSend::%s -- Transaction Lock signatures count: %d/%d, vote hash=%s\n", __func__, nSignatures,
        nSignaturesMax, nVoteHash.ToString());
...
bool CInstantSend::ProcessOrphanTxLockVote(const CTxLockVote& vote)
{
    ...
    int nSignaturesMax = txLockCandidate.txLockRequest.GetMaxSignatures();
    LogPrint(BCLog::INSTANTSEND, "CInstantSend::%s -- Transaction Lock signatures count: %d/%d, vote hash=%s\n", __func__, nSignatures,
        nSignaturesMax, vote.GetHash().ToString());
        ...
        for (const auto& hash : it1->second) {
            if (hash != txHash) {
                ...
                if (it2 != mapTxLockCandidates.end() && it2->second.HasDynodeVoted(vote.GetOutpoint(), vote.GetDynodeOutpoint())) {
                    ...
                    LogPrintf(
                        "CInstantSend::%s -- dynode sent conflicting votes! %s\n", __func__, vote.GetDynodeOutpoint().ToStringShort());
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if (GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            if (itLockCandidate == mapTxLockCandidates.end() || itLockCandidateConflicting == mapTxLockCandidates.end()) {
                ...
                LogPrintf("CInstantSend::ResolveConflicts -- ERROR: Found conflicting completed Transaction Lock, but one of "
                          "txLockCandidate-s is missing, txid=%s, conflicting txid=%s\n",
                    txHash.ToString(), hashConflicting.ToString());
                return false;
            }
            LogPrintf("CInstantSend::ResolveConflicts -- WARNING: Found conflicting completed Transaction Lock, dropping both, txid=%s, "
                      "conflicting txid=%s\n",
                txHash.ToString(), hashConflicting.ToString());
...
void CInstantSend::CheckAndRemove()
{
    ...
    while (itOrphanVote != mapTxLockVotesOrphan.end()) {
        if (itOrphanVote->second.IsTimedOut()) {
            LogPrint(BCLog::INSTANTSEND, "CInstantSend::CheckAndRemove -- Removing timed out orphan vote: txid=%s  dynode=%s\n",
    ...
    while (itVote != mapTxLockVotes.end()) {
        if (itVote->second.IsFailed()) {
            LogPrint(BCLog::INSTANTSEND, "CInstantSend::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  dynode=%s\n",
                itVote->second.GetTxHash().ToString(), itVote->second.GetDynodeOutpoint().ToStringShort());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    while (itDynodeOrphan != mapDynodeOrphanVotes.end()) {
        if (itDynodeOrphan->second < GetTime()) {
            LogPrint(BCLog::INSTANTSEND, "CInstantSend::CheckAndRemove -- Removing timed out orphan dynode vote: dynode=%s\n",
...
{
    LOCK(cs_instantsend);
    return mapLockRequestAccepted.count(hash) || mapLockRequestRejected.count(hash) || mapTxLockVotes.count(hash);
}
...
int CInstantSend::GetConfirmations(const uint256& nTXHash) { return IsLockedInstantSendTransaction(nTXHash) ? nInstantSendDepth : 0; }
...
bool CInstantSend::IsTxLockCandidateTimedOut(const uint256& txHash)
{
    ...
    std::map<uint256, CTxLockCandidate>::iterator itLockCandidate = mapTxLockCandidates.find(txHash);
    if (itLockCandidate != mapTxLockCandidates.end()) {
        return !itLockCandidate->second.IsAllOutPointsReady() && itLockCandidate->second.IsTimedOut();
    }
...
void CInstantSend::UpdatedBlockTip(const CBlockIndex* pindex) { nCachedBlockHeight = pindex->nHeight; }
...
void CInstantSend::SyncTransaction(const CTransaction& tx, const CBlockIndex* pindex, int posInBlock)
{
    ...
    for (const auto& pair : mapTxLockVotesOrphan) {
        if (pair.second.GetTxHash() == txHash) {
            LogPrint(BCLog::INSTANTSEND, "CInstantSend::SyncTransaction -- txid=%s nHeightNew=%d vote %s updated\n", txHash.ToString(),
                nHeightNew, pair.first.ToString());
...
{
    LOCK(cs_instantsend);
    return tfm::format("Lock Candidates: %llu, Votes %llu", mapTxLockCandidates.size(), mapTxLockVotes.size());
}
...
int CTxLockRequest::GetMaxSignatures() const { return tx->vin.size() * COutPointLock::SIGNATURES_TOTAL; }
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    ...
    if (!dnodeman.GetDynodeRank(outpointDynode, nRank, nLockInputHeight, nMinRequiredProtocol)) {
        ...
        LogPrint(BCLog::INSTANTSEND, "CTxLockVote::IsValid -- Can't calculate rank for dynode %s\n", outpointDynode.ToStringShort());
        return false;
    }
...
uint256 CTxLockVote::GetHash() const { return SerializeHash(*this); }

uint256 CTxLockVote::GetSignatureHash() const { return GetHash(); }
...
bool CTxLockVote::IsTimedOut() const { return GetTime() - nTimeCreated > INSTANTSEND_LOCK_TIMEOUT_SECONDS; }
...
bool COutPointLock::AddVote(const CTxLockVote& vote) { return mapDynodeVotes.emplace(vote.GetDynodeOutpoint(), vote).second; }
...
bool COutPointLock::HasDynodeVoted(const COutPoint& outpointDynodeIn) const { return mapDynodeVotes.count(outpointDynodeIn); }
...
bool CTxLockCandidate::IsTimedOut() const { return GetTime() - nTimeCreated > INSTANTSEND_LOCK_TIMEOUT_SECONDS; }
