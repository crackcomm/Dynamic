// Co / pow.ht (c) 2016-2018 Duality Blockchain Solutions Developers
...
struct CDiskBlockPos {
    ...
    std::string ToString() const { return tfm::format("CBlockDiskPos(nFile=%i, nPos=%i)", nFile, nPos); }
};
...
public:
    ...
    std::string ToString() const
    {
        return tfm::format("CBlockIndex(pprev=%p, nHeight=%d, merkle=%s, hashBlock=%s)", pprev, nHeight, hashMerkleRoot.ToString(),
...
/** Return the time it would take to redo the work difference between from and to, assuming the current hashrate corresponds to the
 * difficulty at tip, in seconds. */
