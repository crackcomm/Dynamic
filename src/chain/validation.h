...
#include "net/protocol.h" // For CMessageHeader::MessageStartChars
...
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 0.1 * COIN; // "smallest denom" + X * "denom tails"
 ...
 *
 * @param[out]  state   This may be set to an Error state if any error occurred processing it, including during validation/connection/etc of
 * otherwise unrelated blocks during reorganisation; or it may be set to an Invalid state if pblock is itself invalid (but this is not
 * guaranteed even when the block is checked). If you want to *possibly* get feedback on whether pblock is valid, you must also install a
 * CValidationInterface (see chain/validationinterface.h) - this will have its BlockChecked method called whenever *any* block completes
 * validation.
 * @param[in]   pfrom   The node which we are receiving the block from; it is added to mapBlockSource and may be penalised if the block is
 * invalid.
