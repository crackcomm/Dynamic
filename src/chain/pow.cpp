...
unsigned int GetNextWorkRequired(const INDEX_TYPE pindexLast, const BLOCK_TYPE block, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight + 1 <= params.nUpdateDiffAlgoHeight)
        return UintToArith256(params.powLimit)
            .GetCompact(); // genesis block and first x (nUpdateDiffAlgoHeight) blocks use the default difficulty
    ...
    LogPrint(
        BCLog::POW, "params.AveragingWindowTimespan() = %d    nActualTimespan = %d\n", params.AveragingWindowTimespan(), nActualTimespan);
