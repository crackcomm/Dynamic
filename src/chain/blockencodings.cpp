// Co / params.hc) 2016-2018 Duality Blockchain Solutions Developers
    ...
    for (size_t i = 0; i < extra_txn.size(); i++) {
        ...
        if (idit != shorttxids.end()) {
            if (!have_txn[idit->second]) {
                ...
                if (txn_available[idit->second] && txn_available[idit->second]->GetHash() != extra_txn[i].second->GetHash()) {
                    txn_available[idit->second].reset();
                    mempool_count--;
                    extra_count--;
                }
...
ReadStatus PartiallyDownloadedBlock::FillBlock(CBlock& block, const std::vector<CTransactionRef>& vtx_missing)
{
    ...
    LogPrint(BCLog::CMPCTBLOCK,
        "Successfully reconstructed block %s with %lu txn prefilled, %lu txn from mempool (incl at least %lu from extra pool) and %lu txn "
        "requested\n",
