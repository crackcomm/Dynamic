...
bool GetFluidMintData(const CScript& scriptPubKey, CFluidMint& entry)
{
    ...
    if (vecSplitScript.size() >= 6 && strOperationCode == "OP_MINT") {
        ...
        entry.SovereignAddresses.push_back(
            CharVectorFromString(fluid.GetAddressFromDigestSignature(vecSplitScript[3], messageTokenKey).ToString()));
        entry.SovereignAddresses.push_back(
            CharVectorFromString(fluid.GetAddressFromDigestSignature(vecSplitScript[4], messageTokenKey).ToString()));
        entry.SovereignAddresses.push_back(
            CharVectorFromString(fluid.GetAddressFromDigestSignature(vecSplitScript[5], messageTokenKey).ToString()));

        LogPrintf("GetFluidMintData: strAmount = %s, strTimeStamp = %d, DestinationAddress = %s, Addresses1 = %s, Addresses2 = %s, "
                  "Addresses3 = %s \n",
            strAmount, entry.nTimeStamp, StringFromCharVector(entry.DestinationAddress), StringFromCharVector(entry.SovereignAddresses[0]),
...
CDynamicAddress CFluidMint::GetDestinationAddress() const { return CDynamicAddress(StringFromCharVector(DestinationAddress)); }

CFluidMintDB::CFluidMintDB(size_t nCacheSize, bool fMemory, bool fWipe, bool obfuscate)
    : CDBWrapper(GetDataDir() / "blocks" / "fluid-mint", nCacheSize, fMemory, fWipe, obfuscate)
{
}
