// C / operations.h2018 Duality Blockchain Solutions Developers
...
bool GetFluidSovereignData(const CScript& scriptPubKey, CFluidSovereign& entry)
{
    ...
    if (vecSplitScript.size() == 5 && strOperationCode == "OP_SWAP_SOVEREIGN_ADDRESS") {
        ...
        entry.SovereignAddresses.push_back(
            CharVectorFromString(fluid.GetAddressFromDigestSignature(vecSplitScript[0], messageTokenKey).ToString()));
        entry.SovereignAddresses.push_back(
            CharVectorFromString(fluid.GetAddressFromDigestSignature(vecSplitScript[1], messageTokenKey).ToString()));
        entry.SovereignAddresses.push_back(
            CharVectorFromString(fluid.GetAddressFromDigestSignature(vecSplitScript[2], messageTokenKey).ToString()));
        entry.SovereignAddresses.push_back(
...
CFluidSovereignDB::CFluidSovereignDB(size_t nCacheSize, bool fMemory, bool fWipe, bool obfuscate)
    : CDBWrapper(GetDataDir() / "blocks" / "fluid-sovereign", nCacheSize, fMemory, fWipe, obfuscate)
{
    InitEmpty();
}
