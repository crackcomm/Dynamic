...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int nGenProcLimitCPU = GetArg("-genproclimit", DEFAULT_GENERATE_THREADS_CPU);
    if (request.params.size() > 1)
    ...
    ForceSetArg("-genproclimit", nGenProcLimitCPU);
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
        ...
        case THRESHOLD_ACTIVE: {
            ...
            if (setClientRules.find(vbinfo.name) == setClientRules.end()) {
                ...
                if (!vbinfo.gbt_force) {
                    ...
                    throw JSONRPCError(RPC_INVALID_PARAMETER, tfm::format("Support for '%s' rule requires explicit client support", vbinfo.name));
                }
            }
            break;
        }
    ...
    result.push_back(Pair("bits", tfm::format("%08x", pblock->nBits)));
