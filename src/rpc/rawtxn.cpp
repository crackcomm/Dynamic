...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error("getrawtransaction \"txid\" ( verbose )\n"
...
{
    if (request.fHelp || (request.params.size() != 1 && request.params.size() != 2))
        throw std::runtime_error("gettxoutproof [\"txid\",...] ( blockhash )\n"
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the dynamic address, the numeric value (can be string) is "
            "the " +
            CURRENCY_UNIT +
            " amount\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error("decodescript \"hex\"\n"
                                 ...
                                 "     ,...\n"
                                 "  ],\n"
                                 "  \"p2sh\",\"address\" (string) address of P2SH script wrapping this redeem script (not returned if the "
                                 "script is already a P2SH).\n"
                                 "}\n"
                                 "\nExamples:\n" +
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 3 && !request.params[3].isNull()) {
        static std::map<std::string, int> mapSigHashValues = boost::assign::map_list_of(std::string("ALL"), int(SIGHASH_ALL))(
            std::string("ALL|ANYONECANPAY"), int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))(std::string("NONE"), int(SIGHASH_NONE))(
            std::string("NONE|ANYONECANPAY"), int(SIGHASH_NONE | SIGHASH_ANYONECANPAY))(std::string("SINGLE"), int(SIGHASH_SINGLE))(
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (!AcceptToMemoryPool(mempool, state, std::move(tx), !fBypassLimits, &fMissingInputs, NULL, false, nMaxRawTxFee)) {
            if (state.IsInvalid()) {
                throw JSONRPCError(RPC_TRANSACTION_REJECTED, tfm::format("%i: %s", state.GetRejectCode(), state.GetRejectReason()));
            } else {
                if (fMissingInputs) {
...
static const CRPCCommand commands[] = {
    ...
    {"rawtransactions", "signrawtransaction", &signrawtransaction, false,
