...
CScript _createmultisig_redeemScript(const UniValue& params)
{
    ...
    if ((int)keys.size() < nRequired)
        throw std::runtime_error(
            tfm::format("not enough keys supplied "
    ...
    for (unsigned int i = 0; i < keys.size(); i++) {
        ...
        if (pwalletMain && address.IsValid()) {
            ...
            if (!address.GetKeyID(keyID))
                throw std::runtime_error(
                    tfm::format("%s does not refer to a key", ks));
            CPubKey vchPubKey;
            if (!pwalletMain->GetPubKey(keyID, vchPubKey))
                throw std::runtime_error(
                    tfm::format("no full public key for address %s", ks));
    ...
    if (result.size() > MAX_SCRIPT_ELEMENT_SIZE)
        throw std::runtime_error(
            tfm::format("redeemScript exceeds size limit: %d > %d", result.size(), MAX_SCRIPT_ELEMENT_SIZE));
