    ...
    */
    ...
    if (strCommand == "vote-conf") {
        ...
        if (!fDnFound) {
            ...
            returnObj.push_back(Pair("overall", tfm::format("Voted successfully %d time(s) and failed %d time(s).", nSuccessful, nFailed)));
    ...
    if (strCommand == "vote-alias") {
        ...
        for (const auto& dne : dynodeConfig.getEntries()) {
            ...
            if (!CMessageSigner::GetKeysFromSecret(dne.getPrivKey(), keyDynode, pubKeyDynode)) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", tfm::format("Invalid dynode key %s.", dne.getPrivKey())));
                resultsObj.push_back(Pair(dne.getAlias(), statusObj));
                continue;
            }
