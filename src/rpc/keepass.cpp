...
CKeePassIntegrator::CKeePassIntegrator()
    : sKeyBase64(" ")
    , sKey(" ")
    , sUrl(" ") // Prevent LockedPageManagerBase complaints
{
...
std::string CKeePassIntegrator::CKeePassRequest::getJson() { return requestObj.write(); }
...
std::string CKeePassIntegrator::CKeePassResponse::getStr(const std::string& strName) { return responseObj[strName].get_str(); }
...
void CKeePassIntegrator::doHTTPPost(const std::string& sRequest, int& nStatusRet, std::string& strResponseRet)
{
    ...
    else if (response.nStatus >= 400 && response.nStatus != HTTP_BAD_REQUEST && response.nStatus != HTTP_NOT_FOUND &&
             response.nStatus != HTTP_INTERNAL_SERVER_ERROR)
        throw std::runtime_error(tfm::format("server returned HTTP error %d", response.nStatus));
