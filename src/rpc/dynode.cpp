...
UniValue dynode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "connect") {
        ...
        if (!Lookup(strAddress.c_str(), addr, 0, false))
            throw JSONRPCError(RPC_INTERNAL_ERROR, tfm::format("Incorrect dynode address %s", strAddress));
        ...
        if (!g_connman->IsConnected(CAddress(addr, NODE_NETWORK), CConnman::AllNodes))
            throw JSONRPCError(RPC_INTERNAL_ERROR, tfm::format("Couldn't connect to dynode %s", strAddress));
    ...
    if (strCommand == "outputs") {
        ...
        for (const auto& out : vPossibleCoins) {
            obj.push_back(Pair(out.tx->GetHash().ToString(), tfm::format("%d", out.i)));
        }
    ...
    if (strCommand == "winners") {
        ...
        for (int i = nHeight - nLast; i < nHeight + 20; i++) {
            ...
            if (strFilter != "" && strPayment.find(strFilter) == std::string::npos)
                continue;
            obj.push_back(Pair(tfm::format("%d", i), strPayment));
        }
    ...
    if (strMode == "rank") {
        ...
        for (const auto& dnpair : mapDynodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                if (strFilter != "" && strFilter != tfm::format("%d", dn.nProtocolVersion) &&
